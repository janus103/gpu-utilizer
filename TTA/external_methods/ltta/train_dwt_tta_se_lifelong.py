...
def main():
    ...
    # print('Data evak => {}'.format(args.data_dir.split('/')[-2]))
    ...
    def save_floats_to_csv(float_list, filename):
        """
        주어진 float 리스트를 CSV 파일로 저장하는 함수.
        
        Args:
        float_list (list of float): 저장할 실수들의 리스트.
        filename (str): 생성할 CSV 파일의 이름.
        """
        # 파일을 쓰기 모드로 열기
    ...
    save_floats_to_csv(eval_lst,"mobilexxt_{}_LB_{}_batch_{}/{}_seed_{}.csv".format(args.dwt_bn[1],args.lbatch, args.batch_size, corruption_style,args.seed))
    # imagenet_cont_opt_5_LB_5_batch_256
    # imagenet_cont_opt_5_LB_9_batch_256
    # imagenet_cont_opt_5_LB_9_batch_512
...
def train_one_epoch(
    ...
    def model_copy_from_original(model):
        ...
        # model_copy = model_copy.load_state_dict(model.state_dict())
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if batch_idx != 0 and batch_idx % args.lbatch == 0:
            model.eval()
        ...
        with amp_autocast():
            ...
            if model.get_nll_loss() != None:
                ...
                # print('NLL LOSS ? {}'.format(nll_loss.sum()))
                # loss = nll_loss.sum() * -1000
                # print('NLL LOSS AF ? {}'.format(loss.sum()))
