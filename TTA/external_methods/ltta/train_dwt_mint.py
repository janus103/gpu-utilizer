...
def train_one_epoch(
    ...
    dwt_loss_fn = nn.CrossEntropyLoss().to(device=device)
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            if args.dwt_bn[1] == 0:
                ...
                loss = loss_fn(output, target)
                output = torch.mean(compute_entropy(output)) 
                if args.dwt_bn[3] == 1:
                    loss += -nn.MSELoss()(output, torch.zeros_like(output).detach())
                for idx, freq_out in enumerate(freq_out_lst):
                    ...
                    if args.dwt_level[0] == 1: # DWT LEV이 1인 경우 
                        if args.dwt_bn[2] == 1: # HS 3개만 Entropy를 최소화 시킨다. entropy가 현재 크기 때문에 (e.g., 2.x 정도 됨) 이를 줄이는 것
                            if idx == 1 or idx == 2 or idx == 3:
                                loss += nn.MSELoss()(freq_out, torch.zeros_like(freq_out).detach())
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                if args.vit == True:
                    ...
                    if args.dwt_bn[1] == 1:
                        output, _ = model(input, dwt_quant=args.dwt_quant, is_weight = args.weight_net)
                if args.mvar == True:
                    gt_mvar = output[1]
                    output = output[0]
