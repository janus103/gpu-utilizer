...
def main():
    ...
    # # 파일명
    # filename = 'txt_tta/' + args.resume.split('/')[1] + '.txt'

    # # 'a+' 옵션으로 파일 열기 (파일이 없으면 새로 생성)
    # with open(filename, 'a+') as file:
    #     file.write("Top1: {}\n".format(eval_metrics))

    # print(f"'{filename}' 파일에 내용을 저장했습니다.")

def collect_params_1(model):
...
def train_one_epoch(
    ...
    for param_group in optimizer.param_groups:
        #print('first learning rate in epoch --> ',param_group['lr'])
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            if args.dwt_bn[1] == 0 or args.dwt_level[0] == 0:
                ...
                output, freq_out_lst = model(input, dwt_ratio=target_dwt, ena_dwt_ratio=args.ena_dwt_ratio, dwt_quant=args.dwt_quant, dwt_drop=args.drop_low, is_weight = args.weight_net)
                output = (compute_entropy(output)) 
                #output[output < 1.2] = 0
                output = torch.mean(output) 
                
                
                for idx, freq_out in enumerate(freq_out_lst):
                    # freq_out = compute_entropy(freq_out)
                    # freq_mean = torch.mean(freq_out)#1.8239#torch.mean(freq_out)
                    # freq_std = torch.std(freq_out)#0.3869#torch.std(freq_out)
                    # source_mean = 0.8311
                    # source_std = 0.6198
                    # freq_cal = source_std * ((freq_out - freq_mean)/freq_std) + source_mean
                    # loss = nn.MSELoss()(freq_out, freq_cal.clone().detach())
                    # break

                    freq_out = torch.mean(compute_entropy(freq_out)) 
                    if args.dwt_level[0] == 1: # DWT LEV이 1인 경우 
                        if args.dwt_bn[2] == 0: ## 가장 기본적인 방법이였던 것들. 
                            
                            loss = nn.MSELoss()(output, torch.zeros_like(output).detach()) * 1
                            #loss = output
                            break
                        elif args.dwt_bn[2] == 1: # HS 3개만 Entropy를 최소화 시킨다. entropy가 현재 크기 때문에 (e.g., 2.x 정도 됨) 이를 줄이는 것
                            if idx == 1 or idx == 2 or idx == 3:
                                loss = nn.MSELoss()(freq_out, torch.zeros_like(freq_out).detach()) * 0.5 
                        elif args.dwt_bn[2] == 2: # LL만 Entropy를 최대화 시킨다. 
                            if idx == 0:
                                loss = nn.MSELoss()(freq_out, torch.zeros_like(freq_out).detach()) * 0.5
                                break
                        elif args.dwt_bn[2] == 3: # LL과 HH를 동시에 한다.  
                            if idx == 0:
                                loss = nn.MSELoss()(freq_out, torch.zeros_like(freq_out).detach())* 0.5
                            elif idx == 1 or idx == 2 or idx == 3:
                                loss += nn.MSELoss()(freq_out, torch.zeros_like(freq_out).detach())* 0.1
                    if args.dwt_level[0] == 2:
                        if args.dwt_bn[2] == 1:
                            loss = nn.MSELoss()(output, torch.zeros_like(output).detach()) * 1
                            break
                        elif args.dwt_bn[2] == 0:
                            if idx == 0:
                                loss = nn.MSELoss()(freq_out, torch.zeros_like(freq_out).detach()) * 1  
                            # elif idx == 4 or idx == 8 or idx == 12:
                            #     loss += nn.MSELoss()(freq_out, torch.zeros_like(freq_out).detach()) * 0.125
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            if utils.is_primary(args):
                ...
                #         args,
                #     )
                #     print('Middle Test -> ', eval_metrics)
                #     for param in model.parameters():
...
def validate(
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                if args.dwt_bn[1] == 0 or args.dwt_level[0] == 0:
                    ...
                    output, freq_out = model(input, dwt_quant=args.dwt_quant, is_weight = args.weight_net)
                    LL_lst.append(torch.mean(compute_entropy(freq_out[0])))
                    #loss = loss_fn(output, target)
    ...
    print('LL Entropy (avg) {}'.format(sum(LL_lst) / len(LL_lst)))
