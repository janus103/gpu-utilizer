...
def train_one_epoch(
    ...
    #values = torch.tensor([0.2997,0.3454,0.2807,0.0742])
    # values = torch.tensor([args.ent_lst[0], args.ent_lst[1], args.ent_lst[2], args.ent_lst[3]])
    # gt_entropy = values.repeat(128, 1).cuda()
    print('Hello Training one epoch')
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            if model.get_nll_loss() != None:
                #print('NLL LOSS shape = {}    entropy shape {}'.format(model.get_nll_loss(), mask)) # batch size
                nll_loss = model.get_nll_loss()
                #nll_loss = model.get_nll_loss()[mask]
                #print('PNLL shape   {}'.format(nll_loss.shape))
                loss = nll_loss.mean()
        ...
        # Full Iteration => 391
        if glb_batch_idx == stop_point + start_point:
            print(glb_batch_idx)
            break
    
    # 도수분포 확인 entropy 
    frequency_distribution(entropy_lst)

    model_temp = model.module if hasattr(model, 'module') else model
    torch.save(model_temp.state_dict(), './cont_save/impulse_iter_'+str(glb_batch_idx)+'.pth.tar')
    #torch.save(model_temp.state_dict(), './cont_save/ela_iter_1'+'.pth.tar')
    ...
    with torch.no_grad():
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast(): 
                ...
                entropy_lst += entropy.tolist()
                #mean_lst.append(ANA.analysis(model.nll_loss))
            
            acc_lst, mask = utils.accuracy_custom(output, target, topk=(1, 5))
            acc1, acc5 = acc_lst[0], acc_lst[1]

            # labels_tensor = torch.tensor(mask, dtype=torch.long).view(-1, 1)

            acc_lst_app.append(acc1.item())
        ...
        frequency_distribution(entropy_lst)
        #print('average -> ',sum(mean_lst)/len(mean_lst))
