...
group.add_argument('--lbatch', type=int, default=0, metavar='M',
                    help='Last Batch number !')
group.add_argument('--pbatch', type=int, default=0, metavar='M',
                    help='Position Batch !')
...
def main():
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.ada == 1:
                nll_loss, last_entropy = train_one_epoch(
                    epoch,
                    model,
                    loader_train,
                    optimizer,
            ...
            write_result_lst.append(last_entropy.mean().item())
            
            if True:
                eval_metrics, entropy_mean, cosine_sim_mean = validate(
                    model,
                    loader_eval,
                    validate_loss_fn,
                    args,
...
def train_one_epoch(
    ...
    entropy_lst = []
    check_point = args.pbatch
    for batch_idx, (input, target) in enumerate(loader):
        if batch_idx != check_point:
            continue
        ...
        for batch_idx2 in range(args.lbatch):
            with amp_autocast():
