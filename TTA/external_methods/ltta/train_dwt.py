    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            if args.dwt_bn[2] != 2: 
                ...
                output, h_lst = model(input, dwt_ratio=target_dwt, ena_dwt_ratio=args.ena_dwt_ratio, dwt_quant=args.dwt_quant, dwt_drop=args.drop_low)
                loss = loss_fn(output, target)
                #SM = nn.Softmax(dim=-1)
                #h_output = SM(torch.mean(torch.stack(h_lst, dim=-1).squeeze(-2).squeeze(-3), dim = 1 )).squeeze(1)
                #loss += val_loss_fn(h_output,gt_entropy)
