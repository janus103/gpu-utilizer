...
def validate(args):
    ...
    with torch.no_grad():
        ...
        with amp_autocast():
            ...
            # else:
            #     model(input)
            model(input, is_weight = args.weight_net)
                ...
                if args.vit == False:
                    if args.dwt_bn[1] == 0:
                        ...
                        for idx, freq_out in enumerate(freq_out_lst):
                            #print('Freq out shape -> {}'.format(compute_entropy(freq_out).shape)) # (8,10)
                            #STATISTICS_LST[str(idx)].append(torch.mean(compute_entropy(freq_out)).item())
                            if idx == 0:
                                LL_lst += compute_entropy(freq_out).tolist()
                            elif idx == 1:
                                LH_lst += compute_entropy(freq_out).tolist()
                            elif idx == 2:
                                HL_lst += compute_entropy(freq_out).tolist()
                            elif idx == 3:
                                HH_lst += compute_entropy(freq_out).tolist()
    ...
    values = LL_lst
    print('LL lst length {}'.format(len(LL_lst)))
    print('LH lst length {}'.format(len(LH_lst)))
    print('HL lst length {}'.format(len(HL_lst)))
    print('HH lst length {}'.format(len(HH_lst)))
    if real_labels is not None:
    ...
    return [LL_lst,LH_lst,LH_lst,HH_lst]
...
def main():
    ...
    results_file = 'csv_stat/' + args.results_file
    write_results(results_file=results_file, lists=results)

def write_results(results_file, lists):
    with open(results_file, 'w+', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['LL', 'LH', 'HL', 'HH'])  # 컬럼명을 쓰기 (옵션)
        for f1, f2, f3, f4 in zip(lists[0], lists[1], lists[2], lists[3]):
