...
def main():
    ...
    # NOTE:
    # - This repository uses a custom dataset target format (e.g. `target == [class_idx, extra]`).
    # - The train/val loops in this script expect that structure via the non-prefetcher path
    #   (`target[0]` for class label, `target[1]` for extra meta like DWT-related info).
    # - With prefetcher enabled, timm fast-collate / FastCollateMixup expects integer class targets
    #   and will crash with errors like: `TypeError: an integer is required (got type list)`.
    # For correctness & compatibility, we force prefetcher off in this `train.py` entrypoint.
    if not args.no_prefetcher:
        _logger.warning(
            'Forcing --no-prefetcher for compatibility with this repo (structured targets + mixup).'
        )
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            if args.model.startswith("resnet"):
                if args.model != 'resnet50' and args.model != 'resnet18' and args.model != 'resnet26': 
