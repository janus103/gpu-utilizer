...
def build_model_with_cfg(
    ...
    if model_cfg is None:
        # print('model cfg is none') # Swin Transformer is here
        model = model_cls(**kwargs)
    else:
        print(f'Model CFG : type({type(model_cfg)}) // {model_cfg}')
        print(f'Model kargs : type({type(kwargs)}) // {kwargs}')
        model = model_cls(cfg=model_cfg, **kwargs)
        
    # else:
    #     if model_cfg is None:
    #         model = model_cls(dwt_level=dwt_level, **kwargs)
    #     else:
    #         model = model_cls(cfg=model_cfg, dwt_level=dwt_level,**kwargs)
