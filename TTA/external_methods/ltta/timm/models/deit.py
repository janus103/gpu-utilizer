...
def _create_deit(variant, pretrained=False, distilled=False, dwt_level=[0,0,0], **kwargs):
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
        
    model_cls = VisionTransformerDistilled if distilled else VisionTransformer
    
    if model_cls == VisionTransformer:
        model = build_model_with_cfg(
            model_cls, variant, pretrained,
            pretrained_filter_fn=partial(checkpoint_filter_fn, adapt_layer_scale=True), dwt_level=dwt_level,
            **kwargs)
    else: 
        model = build_model_with_cfg(
            model_cls, variant, pretrained,
...
@register_model
def deit_tiny_patch16_224(pretrained=False, dwt_level=[0,0,0], **kwargs):
    """ DeiT-tiny model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    #print('@@@@@@@@@@@ -> reg model', dwt_level) # delivery completed 
    model_kwargs = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3, dwt_level=dwt_level,  **kwargs)
