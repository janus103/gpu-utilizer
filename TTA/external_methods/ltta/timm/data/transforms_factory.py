...
from timm.data.constants import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD, DEFAULT_CROP_PCT, PACS_PHOTO_DEFAULT_MEAN, PACS_PHOTO_DEFAULT_STD, PACS_ART_DEFAULT_MEAN, PACS_ART_DEFAULT_STD, PACS_CARTOON_DEFAULT_MEAN,\
      PACS_CARTOON_DEFAULT_STD, PACS_SKETCH_DEFAULT_MEAN, PACS_SKETCH_DEFAULT_STD, PACS_TOTAL_DEFAULT_MEAN, PACS_TOTAL_DEFAULT_STD 
from timm.data.aug import rand_augment_transform, augment_and_mix_transform, auto_augment_transform
...
def transforms_imagenet_train(
    ...
    if separate:
        ...
        if enable_aux == True:
            return [primary_tfl, transforms.Compose(primary_tfl + secondary_tfl + final_tfl), final_tfl]
        else:
...
def transforms_imagenet_eval(
    ...
    if crop_mode == 'squash':
        ...
        if scale_size[0] == scale_size[1]:
            ...
            primary_tfl = [ResizeKeepRatio(scale_size)]
        tfl += [transforms.CenterCrop(img_size)]
        primary_tfl += [transforms.CenterCrop(img_size)]
    final_tfl = []
    if use_prefetcher:
        # prefetcher and collate will handle tensor conversion and norm
        tfl += [ToNumpy()]
    ...
    if enable_aux == True:
        return [primary_tfl,transforms.Compose(tfl),final_tfl]
    else:
        return transforms.Compose(tfl)
