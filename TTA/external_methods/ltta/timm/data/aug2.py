...
''' factor => salt prob 0 ~ 1 ''' 
def salt_noise(img, factor, **kwargs):
    _check_args_tf(kwargs)
    im_np = np.array(img)
    num_salt = np.ceil(factor * im_np.size)
    coords = [np.random.randint(0, i, int(num_salt)) for i in im_np.shape]
    im_np[coords[0], coords[1]] = 255
    return Image.fromarray(im_np)

'''#(factor 0 ~ 1)'''
def flip(img, factor, **kwargs): 
    h_target, v_target = factor[0], factor[1]
    image_np = np.array(img)
    if h_target == 1: 
        result_np = np.array(img.transpose(Image.FLIP_LEFT_RIGHT))
    elif h_target == 0:
        result_np = image_np 
    else:
...
def gaussian_noise(img, factor, **kwargs):
    _check_args_tf(kwargs)
    def noise_fn(pil_img):
        #c = [0.04, 0.06, .08, .09, .10][factor - 1]
        c=factor
        img_np = np.array(pil_img) / 255.
        noisy_img = np.clip(img_np + np.random.normal(size=img_np.shape, scale=c), 0, 1) * 255
        return Image.fromarray((noisy_img).astype(np.uint8))
    gaussian_noise_trn = Lambda(noise_fn)
    return gaussian_noise_trn(img)
...
def _gen_random_float(v, min, max):
    if v == -1:
...
def _rotate_level_to_arg(level, _hparams):
    # range [-30, 30]
    level = _gen_random_float(level, _LEVEL_RAND_MIN, _LEVEL_RAND_MAX)
...
def _enhance_noise_level_to_arg(level, _hparams):
    # range [0, 1.9]
    level = _gen_random_float(level, 0, 10)
    return (level / _LEVEL_DENOM) * 0.1,

def _enhance_aux_flip_to_arg(level, _hparams):
    return [0.5, 0.5],

def _enhance_aux_single_flip_to_arg(level, _hparams):
    level = _gen_random_float(level, 0, 4)
    if level <= 0:
        return [0., 0.],
    elif level <= 1:
        return [0., 1.],
...
LEVEL_TO_ARG = {
    ...
    'SaltNoise': _enhance_noise_level_to_arg,
    'GaussianNoise': _enhance_noise_level_to_arg,
    'FlipAux': _enhance_aux_flip_to_arg,
    'FlipSingle': _enhance_aux_single_flip_to_arg,
}
...
def auxiliary_argument_policy(prob, hparams=None):
    policy = [
            ('Brightness', prob, -1), 
            ('Contrast', prob, -1),  
            ('Color', prob, -1),
            ('Sharpness', prob, -1),
            ('AutoContrast', 0.5, -1), # Probability
            ('Invert', 0.5, -1), # Probability
            ('Equalize', 0.5, -1), # Probability
            ('Solarize', prob, -1),
            ('SolarizeAdd', prob, -1),
            ('Posterize', prob, -1), 
            ('SaltNoise', prob, -1),
            ('GaussianNoise', prob, -1),
            ('ShearX', prob, -1),
            ('ShearY', prob, -1),
            ('Rotate', prob, -1),
            ('FlipAux', 0.5, -1) # Probability
    ]
    pc = [AugmentOp(*a, hparams=hparams) for a in policy]
    return pc

def fix_level_auxiliary_argument_policy(prob, level, hparams=None):
    policy = [
            ('Brightness', prob, level), 
            ('Contrast', prob, level),  
            ('Color', prob, level),
            ('Sharpness', prob, level),
            ('AutoContrast', 0.5, -1), # Probability
            ('Invert', 0.5, -1), # Probability
            ('Equalize', 0.5, -1), # Probability
            ('Solarize', prob, level),
            ('SolarizeAdd', prob, level),
            ('Posterize', prob, level), 
            ('SaltNoise', prob, level),
            ('GaussianNoise', prob, level),
    ]
    pc = [AugmentOp(*a, hparams=hparams) for a in policy]
    return pc

def auxliary_single_policy(name:str, mag_level):
    return [AugmentOp(name , 1.0, mag_level, hparams=None)]
...
def auxiliary_augment_transform(config_str, hparams):
    """
