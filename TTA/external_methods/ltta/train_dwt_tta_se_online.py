...
from timm.data import create_dataset, create_loader, resolve_data_config, Mixup, FastCollateMixup, AugMixDataset, MultiAugDataset
from timm.data.aug2 import AugmentOp
...
group.add_argument('--multi-aug', type=int, default=0, metavar='N',
                    help='Return N fixed-augmentation views per sample in train loader (batch expands by factor). 0=disable.')
group.add_argument('--multi-aug-ops', type=str,
                    default='Brightness,Contrast,Color,Sharpness,AutoContrast,Invert,Equalize',
                    help='Comma-separated AugmentOp names to generate fixed views from (e.g. 7 ops => mini-batch x7).')
group.add_argument('--multi-aug-level', type=float, default=-1,
                    help='AugmentOp magnitude/level. -1 uses random magnitude (per-view).')
group.add_argument('--multi-aug-include-original', action='store_true', default=False,
                    help='Include the unaugmented (clean) view as the first view (adds +1 to multi-aug count).')
...
def main():
    ...
    if args.amp:
        if not has_native_amp:
            raise RuntimeError('Native torch.cuda.amp is not available, but --amp was requested.')
        amp_dtype = torch.float16 if args.amp_dtype == 'float16' else torch.bfloat16
        amp_autocast = lambda: torch.cuda.amp.autocast(dtype=amp_dtype)
    if args.distributed:
        _logger.info(
    ...
    # multi-augmentation views (fixed op list) setup (train loader only)
    multi_aug_enabled = args.multi_aug > 0 or args.multi_aug_include_original
    multi_aug_ops = []
    multi_aug_views = 0
    if multi_aug_enabled:
        op_names = [n.strip() for n in args.multi_aug_ops.split(',') if n.strip()]
        if args.multi_aug > 0:
            op_names = op_names[:args.multi_aug]
        multi_aug_ops = [AugmentOp(name, prob=1.0, magnitude=args.multi_aug_level, hparams=None) for name in op_names]
        multi_aug_views = len(multi_aug_ops) + (1 if args.multi_aug_include_original else 0)
        if multi_aug_views < 1:
            raise ValueError('multi-aug enabled but no ops/original view selected; check --multi-aug / --multi-aug-ops.')
    ...
    if multi_aug_enabled:
        dataset_train = MultiAugDataset(
            dataset_train,
            aug_ops=multi_aug_ops,
            include_original=args.multi_aug_include_original,
        )
    ...
    loader_train = create_loader(
        ...
        num_aug_splits=max(1, multi_aug_views) if multi_aug_enabled else 0,
    ...
    def append_to_csv(file_path, lst):
                    ...
                    with open(file_path, 'a+', newline='') as file:
                        ...
                        writer.writerow(lst)
    domain_tag = args.lbatch
    if args.resume == '':
        weight_tag = 'temp_weight_pre'    
    else:
        weight_tag = args.resume.split('.')[-3].split('/')[-2]
    # {args.experiment}
    # file_name = f'csv_stop_point3/{domain_tag}_{weight_tag}.csv'
    if args.experiment == 'COS_RES_50' or args.experiment == 'RESNET_50' or args.experiment == '':
        file_name = f'csv_stop_point3/{domain_tag}_{weight_tag}.csv'
    else:
        file_name = f'{args.experiment}/{domain_tag}_{weight_tag}_{args.seed}.csv'
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            nll_loss, eval_metrics, num_online_batches = train_online_epoch(
                epoch,
                model,
                loader_train,
                optimizer,
            ...
            write_result_lst = [args.lbatch, nll_loss, eval_metrics, num_online_batches]
    ...
    torch.save(save_state, 'model_checkpoint.pth.tar')
    # if args.resume != '':
...
def _split_target_for_dwt(target, device):
    if isinstance(target, (tuple, list)):
        target_cls = target[0]
        target_dwt = target[1] if len(target) > 1 else None
    else:
        target_cls = target
        target_dwt = None
    if hasattr(target_cls, 'to'):
        target_cls = target_cls.to(device)
    return target_cls, target_dwt


def _forward_train_tta(model, input, target_dwt, args):
    if args.model.startswith('vgg') or args.model.startswith('mobile') or args.model.startswith('swin'):
        return model(input)
    return model(
        input,
        dwt_ratio=target_dwt,
        ena_dwt_ratio=args.ena_dwt_ratio,
        dwt_quant=args.dwt_quant,
        dwt_drop=args.drop_low,
        is_weight=args.weight_net,
    )


def _forward_eval_tta(model, input, args):
    if args.model.startswith('vgg') or args.model.startswith('mobile') or args.model.startswith('swin'):
        return model(input)
    if args.model == 'resnet50' or args.model == 'resnet18':
        return model(input, dwt_quant=args.dwt_quant)
    return model(input, dwt_quant=args.dwt_quant, is_weight=0.0)


def _reset_online_state(model, optimizer, initial_model_state, initial_optimizer_state):
    unwrap_model(model).load_state_dict(initial_model_state, strict=True)
...
def train_online_epoch(
    ...
    reset_interval = args.lbatch if args.lbatch and args.lbatch > 0 else 0

    for batch_idx, (input, target) in enumerate(loader):
        if reset_interval and batch_idx > 0 and batch_idx % reset_interval == 0:
            _reset_online_state(model, optimizer, initial_model_state, initial_optimizer_state)
            print('Reset online weights at batch {}'.format(batch_idx))

        data_time_m.update(time.time() - end)
        input = input.to(device)
        target, target_dwt = _split_target_for_dwt(target, device)
        ...
        if args.ada == 1:
            model.train()
            with amp_autocast():
                _forward_train_tta(model, input, target_dwt, args)
                if not hasattr(model, 'get_nll_loss') or model.get_nll_loss() is None:
                    raise RuntimeError('Online L-TTA requires model.get_nll_loss() after the train forward pass.')
                loss = model.get_nll_loss().mean()
        ...
        with torch.no_grad():
            with amp_autocast():
                output = _forward_eval_tta(model, input, args)
        ...
        if batch_idx == last_idx or batch_idx % args.log_interval == 0:
            if utils.is_primary(args):
                _logger.info(
                    ...
                    'Acc@1: {top1.val:#.4g} ({top1.avg:#.4g})  '
                    'Time: {batch_time.val:.3f}s  '
                    'Data: {data_time.val:.3f} ({data_time.avg:.3f})'.format(
                        epoch,
                        batch_idx,
                        len(loader),
                        100. * batch_idx / last_idx if last_idx else 100.,
                        loss=losses_m,
                        top1=top1_m,
    ...
    return losses_m.avg, top1_m.avg, num_batches_per_epoch
...
def validate(
            ...
            with amp_autocast(): 
                if is_par == True:
                    ...
                    if args.model.startswith('vgg') or args.model.startswith('mobile') or args.model.startswith('swin'):
                        ...
                        if args.model == 'resnet50' or args.model == 'resnet18':
                            output = model(input, dwt_quant=args.dwt_quant)
                        else: 
    ...
    if is_par == False:
        if args.model.startswith('resnet') and args.model != 'resnet50':
            # entropy_mean = sum(entropy_lst) / len(entropy_lst)
            # cosine_mean = sum(cosine_sim_lst) / len(cosine_sim_lst)
            # return top1_m.avg, entropy_mean, cosine_mean.item()
