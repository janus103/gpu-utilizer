#!/usr/bin/env python3
"""Compute Feature Space Centroid (FSC) from stem (first conv layer) for each class.

This script computes the mean feature vector (centroid) for each class using only
correctly classified samples from the training dataset. The first conv layer output
(64 channels, 112x112) is pooled to 4x4 and flattened to create 1024-dim features.

For ResNet50:
- First conv layer: 7x7 conv, stride 2, padding 3 -> 64 channels, 112x112 output
- AdaptiveAvgPool2d(4, 4) -> 64 channels, 4x4 output
- Flatten -> 1024-dim vector

Usage:
    python compute_fsc_stem.py --data-dir /path/to/imagenet/train --model resnet50 --pretrained

Output:
    ./FSC/{model_name}_FSC_stem.pth
"""
...
def extract_stem_features_and_predictions(model, stem_extractor, loader, device, amp=False):
    ...
    with torch.no_grad():
        for batch_idx, (images, targets) in enumerate(tqdm(loader, desc="Extracting stem features")):
...
def main():
    ...
    with torch.no_grad():
        ...
        _logger.info(f"Stem feature dimension: {dummy_output.shape[1]} (expected: 1024)")
    ...
    output_path = output_dir / f"{args.model}_FSC_stem.pth"
    torch.save(fsc, output_path)
    
    _logger.info(f"FSC saved to: {output_path}")
    _logger.info(f"Centroid shape: {fsc['centroids'].shape}")
    _logger.info(f"Feature dimension: {fsc['feature_dim']} (from stem conv1 -> pool 4x4 -> flatten)")
