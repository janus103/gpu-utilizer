#!/usr/bin/env python3
"""Phase 1 (ResNet): Direct Aug Classifier — Reference-Free, InstanceNorm + Strided DW.

Architecture (no FSC, no external reference, no diff):
    conv1 → [B, 64, 112, 112]  (no pooling)
        → InstanceNorm2d(64, affine=True)        content removal
        → DW Conv(3×3, s=2) ×5 stages           112→56→28→14→7→4
        → (normed − clean_ref)                   learnable clean center
        → PW Conv(64→64, 1×1)                   channel mixing
...
Usage:
    python train_phase1_resnet.py --data-dir /path/to/imagenet \\
        --model resnet50 --pretrained --train-split val --cache-images
"""
import argparse
import gc
import importlib
...
_logger = logging.getLogger('train_phase1_resnet')


# =============================================================================
# Stem Feature Extractor — ResNet conv1 only, NO pooling
# =============================================================================

class StemFeatureExtractor(nn.Module):
    """Extract raw conv1 features without any pooling.

    Output: ``[B, 64, 112, 112]`` for 224×224 input.
    """
    ...
    def __init__(self, model):
        ...
        self.conv1 = model.conv1  # Conv2d(3, 64, 7, stride=2, padding=3)

    def forward(self, x):
        return self.conv1(x)  # [B, 64, 112, 112]
...
class DirectAugClassifier(nn.Module):
    ...
    Args:
        stem_channels: Channels from stem (64 for ResNet conv1).
        output_channels: Channels after PW conv.
        num_dw_stages: Number of stride-2 DW stages (5 for 112→4).
    ...
    def encode(self, features_spatial: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            features_spatial: ``[B, 64, 112, 112]`` raw conv1 features.

        Returns:
            z_flat: ``[B, feature_dim]`` projected vector.
        """
        normed = self.inst_norm(features_spatial)       # [B, 64, 112, 112]
        reduced = self.dw_stages(normed)                # [B, 64, 4, 4]
        diff = reduced - self.clean_ref                 # [B, 64, 4, 4]
        z = self.pw_conv(diff)                          # [B, 64, 4, 4]
        return z.flatten(1)                             # [B, 1024] = [B, 64 * 4 * 4]
    ...
    def warmstart_clean_ref(
        ...
        for clean_imgs, aug_imgs, labels in loader:
            if count >= num_samples:
                break
            f = stem_extractor(clean_imgs.to(device))   # [B, 64, 112, 112]
...
def train_one_epoch(
    ...
    for batch_idx, (clean_imgs, aug_imgs, labels) in enumerate(loader):
        ...
        with torch.no_grad():
            f_clean = stem_extractor(clean_imgs.to(device))       # [B, 64, 112, 112]
            f_augs_parts = []
            for t in range(T):
                f_augs_parts.append(
                    stem_extractor(aug_imgs[:, t].to(device))
                )
            f_augs_flat = torch.cat(f_augs_parts, dim=0)          # [T*B, 64, 112, 112]
            del f_augs_parts

        # ---- Encode directly (no diff, no reference!) ----
        f_all = torch.cat([f_clean, f_augs_flat], dim=0)          # [(1+T)*B, 64, 112, 112]
...
parser = argparse.ArgumentParser(description='Phase 1 (ResNet): Direct Aug Classifier')
...
group.add_argument(
    ...
    help='Groups per batch (each = 1 clean + 7 aug). '
         'ResNet 112×112 features use more memory; 32 recommended.',
)
...
group.add_argument('--wandb-project', default='phase1-direct-resnet', type=str)
group.add_argument(
...
def main():
    ...
    # =========================================================================
    # Stem extractor (frozen, conv1 only, NO pooling)
    # =========================================================================
    stem_extractor = StemFeatureExtractor(backbone).to(device).eval()
    for p in stem_extractor.parameters():
        p.requires_grad = False
    ...
    _logger.info(f'Stem (conv1): [{stem_channels}, {stem_h}, {stem_w}]')
    ...
    _logger.info(
        ...
        f'PW={n_pw:,}, feat_dim={aug_classifier.feature_dim})'
    )
