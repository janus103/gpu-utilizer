...
class PerSampleAugmentDataset(torch.utils.data.Dataset):
    """Wrap a dataset to emit multiple augmented views of the same sample."""

    def __init__(self, dataset, n_aug: int):
        assert n_aug > 1
...
group.add_argument('--wo-config', type=int, nargs=3, default=[0, 0, 0], metavar=('PWC1', 'DW_DEPTH', 'PWC2'),
...
group.add_argument('--n-aug', type=int, default=1, metavar='N',
                   help='Number of augmented views per sample. >1 enables per-sample multi-augmentation batching.')
...
def main():
    ...
    if args.naflex_loader:
        ...
        if n_aug_mode:
            ...
            dataset_train = PerSampleAugmentDataset(dataset_train, n_aug=args.n_aug)
        else:
            collate_fn = None
            if mixup_active:
                if args.prefetcher:
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        cls_losses_m.update(result.get('cls_loss', loss).item() * accum_steps, batch_size)
        if 'g_loss' in result:
            g_losses_m.update(result['g_loss'].item() * accum_steps, batch_size)
        ...
        if update_idx % args.log_interval == 0 or last_batch:
            ...
            if utils.is_primary(args):
                ...
                if g_losses_m.count > 0:
                    log_msg += (
                        f'CLoss: {cls_losses_m.val:#.3g} ({cls_losses_m.avg:#.3g})  '
                        f'GLoss: {g_losses_m.val:#.3g} ({g_losses_m.avg:#.3g})  '
                    )
                log_msg += (
