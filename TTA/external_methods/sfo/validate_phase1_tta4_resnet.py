#!/usr/bin/env python3
"""Phase-1 Clean-Relative TTA for ResNet50: ImageNet-C evaluation.

Uses Phase-1's DirectAugClassifier with **clean-relative** losses.
Instead of blindly minimizing metrics toward 0, targets measured clean baselines.

Clean-Relative Loss terms:
    --w-clean-rel:  relu(p_clean_ref - p_clean) — push p_clean toward clean level only
    --w-kl:         KL(aug_dist || clean_dist) — match full clean distribution
    --w-dist-rel:   relu(dist - dist_ref) — reduce dist only above clean level
    --w-znorm-rel:  relu(znorm - znorm_ref) — reduce znorm only above clean level
...
Also supports original absolute losses (--w-clean, --w-dist, --w-znorm, etc.)
and trust region (--lam) / gradient clipping (--clip-grad).

TTA modes:
    static:   reset conv1 to pretrained each mini-batch (independent).
    continue: carry adapted conv1 across batches (cumulative).

ResNet50 clean baseline (measured on ImageNet val):
    p_clean=0.537, znorm=2.10, dist=2.08
    clean_dist=[0.5373, 0.0046, 0.1370, 0.3027, 0.0000, 0.0000, 0.0183, 0.0000]

Usage:
    python validate_phase1_tta3_resnet.py \\
        ...
        --tta-mode static --inner-lr 0.005 --inner-steps 3 \\
        --w-clean-rel 1.0 --w-dist-rel 1.0 --w-znorm-rel 1.0 --lam 1.0
"""
...
_logger = logging.getLogger("validate_phase1_tta3_resnet")

RESNET50_CLEAN_DIST = [0.5373, 0.0046, 0.1370, 0.3027, 0.0000, 0.0000, 0.0183, 0.0000]
...
def phase1_tta_inner_loop(
    ...
    inner_steps: int,
    w_clean: float = 0.0,
    w_clean_rel: float = 0.0,
    w_kl: float = 0.0,
    w_entropy: float = 0.0,
    w_dist: float = 0.0,
    w_dist_rel: float = 0.0,
    w_znorm: float = 0.0,
    w_znorm_rel: float = 0.0,
    w_repel: float = 0.0,
    momentum: float = 0.0,
    lam: float = 0.0,
    clip_grad: float = 0.0,
    ...
    clean_dist_ref: torch.Tensor = None,
    clean_rel_mode: str = "relu",
    normalize_rel_loss: bool = True,
    guard_tta: bool = True,
    ...
    """Adapt conv1.weight using clean-relative Phase-1 signals.

    New clean-relative losses:
        w_clean_rel: relu(p_clean_ref - p_clean).mean()
        w_kl:        KL(aug_dist || clean_dist_ref)
        w_dist_rel:  relu(dist - dist_ref).mean()
        w_znorm_rel: relu(znorm - znorm_ref).mean()
    """
    ...
    def _relative_progress_score(
        ...
        """Score candidate by relative progress from batch-start diagnostics."""
        p_gain = (p_clean - p_clean_base) / (p_clean_ref + eps)
        dist_rise = 0.0
        if dist_base > 0:
            dist_rise = max(0.0, (dist - dist_base) / (dist_base + eps))
        znorm_rise = 0.0
        if znorm_base > 0:
            znorm_rise = max(0.0, (znorm - znorm_base) / (znorm_base + eps))
        return p_gain - score_dist_penalty * dist_rise - score_znorm_penalty * znorm_rise

    # Diagnostics: before
    p_clean_before, entropy_before, znorm_before, dist_before = _collect_diag()

    # Optional early skip: already close to clean regime, avoid unnecessary drift.
    skip_adapt = (
        skip_clean_like
    ...
    best_score = 0.0
    # Use both clean-reference limits and batch-start-relative limits.
    dist_limit_ref = dist_ref * (1.0 + max_dist_rise)
    znorm_limit_ref = znorm_ref * (1.0 + max_znorm_rise)
    dist_limit_batch = dist_before * (1.0 + max_dist_rise)
    znorm_limit_batch = znorm_before * (1.0 + max_znorm_rise)
    dist_limit = max(dist_limit_ref, dist_limit_batch)
    znorm_limit = max(znorm_limit_ref, znorm_limit_batch)
    ...
    for _k in range(inner_steps):
        ...
        # --- Clean-relative losses ---
        if w_clean_rel > 0:
            p_clean_cur = F.softmax(aug_output, dim=1)[:, 0]
            if normalize_rel_loss:
                p_gap = (p_clean_ref - p_clean_cur) / (p_clean_ref + eps)
            else:
                p_gap = p_clean_ref - p_clean_cur
            if clean_rel_mode == "mse":
                loss = loss + w_clean_rel * p_gap.pow(2).mean()
            else:
                loss = loss + w_clean_rel * F.relu(p_gap).mean()

        if w_kl > 0 and clean_dist_ref is not None:
            probs = F.softmax(aug_output, dim=1).clamp(min=eps)
            target = clean_dist_ref.unsqueeze(0).expand_as(probs).clamp(min=eps)
            kl_aug_clean = (probs * (torch.log(probs) - torch.log(target))).sum(dim=1).mean()
            loss = loss + w_kl * kl_aug_clean

        if w_dist_rel > 0:
            if normalize_rel_loss:
                dist_gap = (dist_output - dist_ref) / (dist_ref + eps)
            else:
                dist_gap = dist_output - dist_ref
            loss = loss + w_dist_rel * F.relu(dist_gap).mean()

        if w_znorm_rel > 0:
            zn = z.norm(dim=1)
            if normalize_rel_loss:
                znorm_gap = (zn - znorm_ref) / (znorm_ref + eps)
            else:
                znorm_gap = zn - znorm_ref
            loss = loss + w_znorm_rel * F.relu(znorm_gap).mean()
        ...
        cand_score = _relative_progress_score(
            p_clean_cur, dist_cur, znorm_cur,
            p_clean_before, dist_before, znorm_before,
        )
    ...
    # Restore best accepted weight if guard is enabled.
    if guard_tta:
...
def _evaluate_corruption(
    ...
    device: torch.device,
    clean_dist_tensor: torch.Tensor = None,
    ...
    for batch_idx, (images, target) in enumerate(loader):
        ...
        diag = phase1_tta_inner_loop(
            ...
            clean_dist_ref=clean_dist_tensor,
            clean_rel_mode=args.clean_rel_mode,
            normalize_rel_loss=not args.disable_rel_norm,
...
def _generate_results_filename(args: argparse.Namespace) -> str:
    parts = ["p1tta4", args.tta_mode, f"lr{args.inner_lr}", f"s{args.inner_steps}"]
    for name, val in [("c", args.w_clean), ("cr", args.w_clean_rel), ("kl", args.w_kl),
                      ("e", args.w_entropy),
                      ("d", args.w_dist), ("dr", args.w_dist_rel),
                      ("z", args.w_znorm), ("zr", args.w_znorm_rel),
                      ("r", args.w_repel)]:
        if val > 0:
            parts.append(f"{name}{val}")
    if args.w_clean_rel > 0 and args.clean_rel_mode != "relu":
        parts.append(args.clean_rel_mode)
    if args.lam > 0:
...
def _save_results(
    ...
    with filepath.open("w") as f:
        ...
        f.write(f"# clean_rel_mode: {args.clean_rel_mode}\n")
        ...
        f.write(f"# disable_rel_norm: {args.disable_rel_norm}\n")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Phase-1 Clean-Relative TTA v4 for ResNet50 on ImageNet-C",
    )
    ...
    g = parser.add_argument_group("Original Absolute Losses")
    g.add_argument("--w-clean", default=0.0, type=float,
                   help="P(clean) maximization weight (absolute, toward 1.0).")
    ...
    g.add_argument("--w-dist", default=0.0, type=float,
                   help="dist_output minimization weight (absolute, toward 0).")
    g.add_argument("--w-znorm", default=0.0, type=float,
                   help="z-norm minimization weight (absolute, toward 0).")
    g.add_argument("--w-repel", default=0.0, type=float,
                   help="Repel from original features weight.")

    g = parser.add_argument_group("Clean-Relative Losses")
    g.add_argument("--w-clean-rel", default=0.0, type=float,
                   help="relu(p_clean_ref - p_clean) or MSE: push p_clean toward clean level.")
    g.add_argument("--clean-rel-mode", default="relu", choices=["relu", "mse"],
                   help="relu: F.relu(p_clean_ref - p_clean); mse: (p_clean_ref - p_clean)^2.")
    g.add_argument("--w-kl", default=0.0, type=float,
                   help="KL(aug_output || clean_dist_ref): match full clean distribution.")
    g.add_argument("--w-dist-rel", default=0.0, type=float,
                   help="relu(dist - dist_ref): reduce dist only above clean level.")
    g.add_argument("--w-znorm-rel", default=0.0, type=float,
                   help="relu(znorm - znorm_ref): reduce znorm only above clean level.")

    g = parser.add_argument_group("Clean Reference Values")
    g.add_argument("--znorm-ref", default=2.10, type=float,
                   help="Clean znorm reference (ResNet50 default: 2.10).")
    g.add_argument("--dist-ref", default=2.08, type=float,
                   help="Clean dist reference (ResNet50 default: 2.08).")
    g.add_argument("--p-clean-ref", default=0.537, type=float,
                   help="Clean p_clean reference (ResNet50 default: 0.537).")
    ...
    g.add_argument("--max-dist-rise", default=0.15, type=float,
                   help="Guard ratio used for both clean-ref and batch-start dist limits.")
    g.add_argument("--max-znorm-rise", default=0.15, type=float,
                   help="Guard ratio used for both clean-ref and batch-start znorm limits.")
    ...
    g.add_argument("--score-dist-penalty", default=0.5, type=float,
                   help="Penalty weight for relative dist rise (vs batch-start) in step score.")
    g.add_argument("--score-znorm-penalty", default=0.5, type=float,
                   help="Penalty weight for relative znorm rise (vs batch-start) in step score.")
    g.add_argument("--min-score-improve", default=1e-4, type=float,
                   help="Minimum score gain required to accept a step.")
    g.add_argument("--disable-rel-norm", action="store_true",
                   help="Use unnormalized clean-relative losses.")
    ...
    g.add_argument("--results-dir", default="./results_phase1_tta4", type=str)
...
def main() -> None:
    ...
    all_weights = [args.w_clean, args.w_clean_rel, args.w_kl, args.w_entropy,
                   args.w_dist, args.w_dist_rel, args.w_znorm, args.w_znorm_rel,
                   args.w_repel]
    if sum(1 for w in all_weights if w > 0) == 0:
    ...
    # ---- Clean distribution reference tensor ----
    clean_dist_tensor = torch.tensor(RESNET50_CLEAN_DIST, device=device)
    clean_dist_tensor = clean_dist_tensor.clamp(min=1e-8)
    clean_dist_tensor = clean_dist_tensor / clean_dist_tensor.sum()
    _logger.info(
        "Clean refs: p_clean=%.3f, znorm=%.2f, dist=%.2f",
    ...
    loss_desc = []
    for name, val in [("clean", args.w_clean), ("clean_rel", args.w_clean_rel),
                      ("kl", args.w_kl), ("entropy", args.w_entropy),
                      ("dist", args.w_dist), ("dist_rel", args.w_dist_rel),
                      ("znorm", args.w_znorm), ("znorm_rel", args.w_znorm_rel),
                      ("repel", args.w_repel)]:
        if val > 0:
    ...
    for corr_idx, corruption in enumerate(corruptions):
        ...
        pre_t1, pre_t5, ada_t1, ada_t5, desc = _evaluate_corruption(
            ...
            clean_dist_tensor=clean_dist_tensor,
        )
