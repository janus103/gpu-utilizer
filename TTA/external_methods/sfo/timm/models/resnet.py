...
from .vision_transformer import ChannelAttention, SpatialAttention

__all__ = ['ResNet', 'BasicBlock', 'Bottleneck', 'WhiteningObserver', 'SEModule', 'SAModule', 'COVModule', 'SAModule2']  # model_registry will add each entrypoint fn to this

class COVModule(nn.Module):
    def __init__(self, channels=64, extention=1):
        super(COVModule, self).__init__()
        self.eps = 1e-9
        self.expansion = 1 * 2
        self.fc1 = nn.Conv2d(channels, int(channels * extention), kernel_size=1)
        self.gate = nn.Softplus(beta=1, threshold=20)

    def forward(self, x):
        # x.mean(dim=1, keepdim=True)
        x_attn = self.fc1(x_attn)
        x_attn = self.gate(x_attn)

        if self.training:
            # x_attn: (B, C, H, W) -> channel-wise element stats: (B, 1, H, W)
            x_mean = x_attn.mean(dim=1, keepdim=True)
            #이건 기본 설정상 샘플 보정이 들어가므로, 채널 수가 작거나 1이면 불안정하다.
            #보통 이런 regularization에는 아래처럼 쓴다.
            x_var = x_attn.var(dim=1, keepdim=True, unbiased=False)
            
            # 아래에는 x_mean와 x_var의 모든 element가 각각 1이 되도록 하라. 
            g_loss = ((x_mean - 1.0) ** 2).mean() + ((x_var - 1.0) ** 2).mean()

            return (x * x_attn), g_loss

        else: # Validation 
            x_mean = x_attn.mean(dim=1, keepdim=True)
            return (x * x_attn), None


class SEModule(nn.Module): # Channel Attention Module = Squeeze-and-Excitation Module
    ...
    def forward(self, x):
        ...
        var = x[:, 1::2]  # 모든 배치의 홀수 위치 (표준편차)

        if self.training:
            g_loss = (-torch.log(self._gaussian_dist_pdf(mean, var)) / 2).mean()
            return (module_input * mean), g_loss

        else: # Validation 
            return (module_input * mean), None
        
    
    def _gaussian_dist_pdf(self, mean, var):
        gt = 1.0
        var = var.clone() + self.eps
        # print(f"mean: {mean}")
        # print(f"var: {var}")
        pdf_value = torch.exp(- (mean - gt) ** 2.0 / var / 2.0) / torch.sqrt(2.0 * np.pi * var)
        return torch.clamp(pdf_value, min=self.eps)

class SAModule(nn.Module):
    """ Original CBAM spatial attention module
    """
    def __init__(self, kernel_size=7, norm_type=0):
        super(SAModule, self).__init__()
        self.eps = 1e-9
        self.expansion = 1 * 2
        self.conv = nn.Conv2d(2, self.expansion, kernel_size, stride=1, padding=kernel_size // 2, bias=False)
        ...
        if norm_type == 0:
            ...
            # self.norm = nn.LayerNorm(self.expansion)
    ...
    def forward(self, x):
        ...
        x_attn = self.norm(self.conv(x_attn))
        x_attn = self.gate(x_attn)
        
        mean = x_attn[:, 0:1]  # (B, 1, H, W) — 첫 번째 채널
        var  = x_attn[:, 1:2]  # (B, 1, H, W) — 두 번째 채널
...
class SAModule2(nn.Module):
    ...
    def _gaussian_dist_pdf(self, mean, var):
        gt = 0.5
        # Enforce strictly positive variance for numerical stability.
        # SAModule2 uses tanh gate, so raw `var` can be negative.
        var = F.softplus(var.clone()) + self.eps
        pdf_value = torch.exp(- (mean - gt) ** 2.0 / var / 2.0) / torch.sqrt(2.0 * np.pi * var)
        return torch.clamp(pdf_value, min=self.eps)
...
class WhiteningObserver(nn.Module):
    # def __init__(self, in_channels: int, out_channels: int, config: List[int] = None):
    def __init__(self, in_channels: int, config: List[int] = None):
        super().__init__()
        # self.in_channels = in_channels
        # self.out_channels = out_channels

        # if config is None:
        #     config = [out_channels, 1, out_channels]

        pwc1_out, dw_depth, pwc2_out = config
        self.use_pwc1 = pwc1_out > 0
        self.use_dwc = dw_depth > 0
        self.use_pwc2 = pwc2_out > 0

        cur_channels = in_channels

        if self.use_pwc1:
            self.pwc1 = nn.Sequential(
                nn.Conv2d(cur_channels, pwc1_out, kernel_size=1, bias=False),
                nn.BatchNorm2d(pwc1_out),
                nn.ReLU(inplace=True),
            )
            cur_channels = pwc1_out

        if self.use_dwc:
            dwc_layers = []
            for _ in range(dw_depth):
                dwc_layers.extend([
                    nn.Conv2d(
                        cur_channels, cur_channels,
                        kernel_size=7, stride=2, padding=3,
                        groups=cur_channels, bias=False,
                    ),
                    nn.BatchNorm2d(cur_channels),
                    nn.ReLU(inplace=True),
                ])
            self.dwc = nn.Sequential(*dwc_layers)

        if self.use_pwc2:
            self.pwc2 = nn.Sequential(
                nn.Conv2d(cur_channels, pwc2_out, kernel_size=1, bias=False),
                nn.BatchNorm2d(pwc2_out),
                nn.ReLU(inplace=True),
            )
            cur_channels = pwc2_out

        self.instance_norm = nn.InstanceNorm2d(cur_channels, affine=False)

        cov_vec_size = cur_channels * (cur_channels - 1) // 2
        self.fc = nn.Linear(cov_vec_size, cov_vec_size * 2)
        self.sigmoid = nn.Sigmoid()
        self.gt = 0.0
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # TODO 1: Channel-wise covariance matrix
        B, C, H, W = x.shape
        flat = x.view(B, C, -1)                        # (B, C, H*W)
        mean = flat.mean(dim=2, keepdim=True)           # (B, C, 1)
        centered = flat - mean                          # (B, C, H*W)
        cov = torch.bmm(centered, centered.transpose(1, 2)) / (H * W - 1)  # (B, C, C)

        # TODO 2: Upper-triangular elements → vector
        row_idx, col_idx = torch.triu_indices(C, C, offset=1, device=x.device)
        cov_vector = cov[:, row_idx, col_idx]           # (B, C*(C-1)/2)

        cov_vector = self.fc(cov_vector)                # (B, C*(C-1))
        cov_vector = self.sigmoid(cov_vector)
        mean = cov_vector[:, ::2]  # 모든 배치의 짝수 위치 (평균)
        var = cov_vector[:, 1::2]   # 모든 배치의 홀수 위치 (분산)

        # Gaussian PDF (variance form):
        # pdf = exp(-(x - μ)² / (2σ²)) / sqrt(2π·σ²)
        # pdf_value = torch.exp(-(mean - self.gt) ** 2.0 / (2.0 * var)) / torch.sqrt(2.0 * math.pi * var)
        # pdf_value = torch.clamp(pdf_value, min=1e-10)
        # g_loss = (-torch.log(pdf_value) / 2).mean()

        # Gaussian NLL (log-space for numerical stability):
        # -log p(x|μ,σ²) = (x-μ)²/(2σ²) + 0.5*log(σ²) + 0.5*log(2π)
        var_clamped = torch.clamp(var, min=1e-6)
        nll = (mean - self.gt) ** 2.0 / (2.0 * var_clamped) + 0.5 * torch.log(var_clamped)
        g_loss = nll.mean()
        
        # Loss to drive mean toward zero, averaged over batch
...
class StemAutoEncoder(nn.Module):
    """Simple fully-connected autoencoder for stem conv kernel stats (mean/var)."""
...
class ResNet(nn.Module):
    ...
    def __init__(
            ...
            device=None,
            dtype=None,
            stem_ae_latent_size: Optional[int] = None,
            stem_ae_checkpoint: str = '',
            wo_config: Optional[List[int]] = None,
        ...
        Args:
            ...
            use_se_module (bool): Attach SEModule after conv1 for Gaussian TTA pre-training (default False)
            use_sam_module (bool): Attach SAModule after conv1 for Gaussian TTA pre-training (default False)
        """
        super().__init__()
        dd = {'device': device, 'dtype': dtype}
        ...
        self.reverse_se_sam = reverse_se_sam
        ...
        # WhiteningObserver (optional, for resnet26/resnet50)
        self.wo_config = wo_config
        self.use_wo = wo_config is not None and wo_config != [0, 0, 0]
        if self.use_wo:
            self.whitening_observer = WhiteningObserver(in_channels=inplanes, config=wo_config)

        # SEModule for Gaussian TTA pre-training (optional)
        self.use_se_module = use_se_module
        self.nll_loss = None
        if self.use_se_module:
            self.se_module = SEModule(channels=inplanes)

        self.nll_sam_loss = None
        if use_sam_module != -1 and use_sam_module <= 4:
            self.use_sam_module = True
            self.sam_module = SAModule(norm_type=use_sam_module)
        else:
            self.use_sam_module = False

        self.parallel_attention = parallel_attention
        self.var_feature = var_feature
        self.attn_detach = False
        self.attn_residual = False
        self.aug_classifier = None
        self.deconv_adapter = None
        if parallel_attention:
            self.channel_attn = ChannelAttention(channels=inplanes)
            self.spatial_attn = SpatialAttention(
                kernel_size=sam_kernel_size,
                norm_type=sam_norm_type,
                channels=inplanes,
        ...
        # Stem AE (optional)
        self.stem_ae: Optional[StemAutoEncoder] = None
        self.stem_ae_latent_size: Optional[int] = stem_ae_latent_size
        self.stem_ae_checkpoint: str = stem_ae_checkpoint
        self.stem_ae_eps: float = 1e-6
        if stem_ae_latent_size is not None or stem_ae_checkpoint:
            stem_conv = self._get_stem_conv()
            if not isinstance(stem_conv, nn.Conv2d):
                raise TypeError("Stem AE currently supports Conv2d stem.")
            weight = stem_conv.weight
            out_ch, in_ch = weight.shape[0], weight.shape[1]
            input_dim = 2 * out_ch * in_ch
            ckpt = None
            ckpt_latent = None
            if stem_ae_checkpoint:
                ckpt = torch.load(stem_ae_checkpoint, map_location='cpu')
                ckpt_latent = ckpt.get('latent_size', None)
            latent_dim = stem_ae_latent_size or ckpt_latent
            if latent_dim is None:
                raise ValueError("stem_ae_latent_size must be provided or present in checkpoint when using stem AE.")
            self.stem_ae = StemAutoEncoder(input_dim=input_dim, latent_dim=latent_dim)
            if ckpt is not None and 'state_dict' in ckpt:
                self.stem_ae.load_state_dict(ckpt['state_dict'])
            self.stem_ae_latent_size = latent_dim
            self.stem_ae.to(weight.device)
    ...
    def _get_stem_conv(self) -> nn.Conv2d:
        if isinstance(self.conv1, nn.Conv2d):
            return self.conv1
        if isinstance(self.conv1, nn.Sequential) and len(self.conv1) and isinstance(self.conv1[0], nn.Conv2d):
            return self.conv1[0]
        raise TypeError("Stem AE expects Conv2d stem.")

    def _apply_stem_ae(self):
        if self.stem_ae is None:
            return
        stem_conv = self._get_stem_conv()
        weight = stem_conv.weight
        out_ch, in_ch = weight.shape[0], weight.shape[1]
        input_dim = 2 * out_ch * in_ch
        if self.stem_ae.encoder[0].in_features != input_dim:
            return

        means = weight.mean(dim=(2, 3))  # [out, in]
        vars_ = weight.var(dim=(2, 3), unbiased=False)  # [out, in]
        feat = torch.cat([means.flatten(), vars_.flatten()], dim=0).unsqueeze(0).to(weight.device)

        with torch.no_grad():
            recon, _ = self.stem_ae(feat)
        recon = recon.squeeze(0)
        recon_means = recon[:out_ch * in_ch].view(out_ch, in_ch)
        recon_vars = recon[out_ch * in_ch:].view(out_ch, in_ch)

        eps = self.stem_ae_eps
        new_weight = weight.detach().clone()
        for o in range(out_ch):
            for i in range(in_ch):
                w = weight[o, i]
                cur_mean = w.mean()
                cur_var = w.var(unbiased=False)
                normed = (w - cur_mean) / torch.sqrt(cur_var + eps)
                target_std = torch.sqrt(torch.clamp(recon_vars[o, i], min=eps))
                new_weight[o, i] = normed * target_std + recon_means[o, i]

        stem_conv.weight.data.copy_(new_weight)
    ...
    def _apply_parallel_attention(self, x: torch.Tensor) -> torch.Tensor:
        ...
        sa_input = x.detach() if self.attn_detach else x
    ...
    def forward_features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.stem_ae is not None:
            self._apply_stem_ae()

        raw_input = x
        x = self.conv1(x)

        if self.aug_classifier is not None:
            with torch.no_grad():
                self.aug_classifier.eval()
                aug_logits = self.aug_classifier(raw_input)

            s1 = self.aug_classifier.get_stage1_feature()         # (B, 32, 112, 112)
            x = x + s1.mean(dim=1, keepdim=True)                 # (B, 1, 112, 112) broadcast
        ...
        elif self.reverse_se_sam:
            if self.use_sam_module:
                x, sam_loss = self.sam_module(x)
                if self.training and sam_loss is not None:
                    self.nll_sam_loss = sam_loss
                    print(f"nll_sam_loss: {sam_loss}")
                else:
                    self.nll_sam_loss = None

            if self.use_se_module:
                x, se_loss = self.se_module(x)
                if self.training and se_loss is not None:
                    self.nll_loss = se_loss
                    print(f"nll_loss: {se_loss}")
                else:
                    self.nll_loss = None
        else:
            if self.use_se_module:
    ...
    def get_nll_loss(self) -> Optional[torch.Tensor]:
        return self.nll_loss

    def get_nll_sam_loss(self) -> Optional[torch.Tensor]:
        return self.nll_sam_loss
...
@register_model
def resnet50_ae(pretrained: bool = False, **kwargs) -> ResNet:
    """ResNet-50 with optional stem autoencoder adaptation."""
    model_args = dict(block=Bottleneck, layers=(3, 4, 6, 3))
    kwargs.setdefault("pretrained_strict", False)  # <- 추가
    # keep variant 'resnet50' for default cfg / weights compatibility
    return _create_resnet('resnet50', pretrained, **dict(model_args, **kwargs))
