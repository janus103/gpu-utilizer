...
class ChannelAttention(nn.Module): # Channel Attention Module = Squeeze-and-Excitation Module
    ...
    def __init__(self, channels=64):
        ...
        self.relu2 = nn.ReLU(inplace=True) # Original Channel Attention Module have sigmoid activation function
    ...
    def forward(self, x):
        ...
        # # mode 2: 
        # attn = self.fc2(x) # scaling 
        # return attn    

# class SpatialAttention(nn.Module):

#     def __init__(self, kernel_size=3, channels=768, **kwargs):
#         super(SpatialAttention, self).__init__()
#         self.channels = channels
#         self.num_class_of_prop = 10
#         self.reduction_conv = nn.Conv2d(self.channels, self.num_class_of_prop, 1, stride=1, padding='same', bias=False)
#         self.norm_reduction = nn.BatchNorm2d(self.num_class_of_prop)
#         self.relu_reduction = nn.ReLU(inplace=True)
#         self.conv = nn.Conv2d(self.num_class_of_prop, self.num_class_of_prop, kernel_size, stride=1, padding='same', bias=False)
#         self.norm = nn.BatchNorm2d(self.num_class_of_prop)

#     def forward(self, x):
#         B, C, H, W = x.shape
#         x_attn = self.relu_reduction(self.norm_reduction(self.reduction_conv(x)))
#         logits = self.norm(self.conv(x_attn))                         # (B, K, H, W)
#         self._logits = logits
#         x_attn = F.softmax(logits, dim=1)                             # (B, K, H, W)

#         K = self.num_class_of_prop
#         # straight-through one-hot: argmax in forward, softmax gradient in backward
#         one_hot = F.one_hot(x_attn.argmax(dim=1), K).permute(0, 3, 1, 2).to(x_attn.dtype)  # (B, K, H, W)
#         one_hot = one_hot - x_attn.detach() + x_attn  # straight-through trick

#         # scale per class: class 0 -> 1.0, class 1 -> (K-1)/K, ..., class K-1 -> 1/K
#         scale = torch.arange(K, 0, -1, device=x.device, dtype=x.dtype) / K  # (K,)
#         mask = (one_hot * scale[None, :, None, None]).sum(dim=1, keepdim=True)  # (B, 1, H, W)
#         self._last_mask = mask.detach()

#         return x, mask


class MoEChannelAttention(nn.Module):
    """Mixture-of-Experts Channel Attention via weight aggregation.

    Maintains ``num_experts`` sets of SE-style weights (fc1, fc2).
    Before the forward pass, these K weight sets are aggregated into a
    single fc1/fc2 using the AugClassifier probability vector:

        W_fc1 = Σ_k  p_k · W_fc1^{(k)}
        W_fc2 = Σ_k  p_k · W_fc2^{(k)}

    The aggregated weights form a single channel attention layer through
    which the input passes once.  This is analogous to Soft MoE — the
    computation cost equals a single expert, but the effective weights
    are input-conditioned.
    """

    def __init__(self, channels=64, num_experts=8, reduction=8):
        super().__init__()
        self.channels = channels
        self.num_experts = num_experts
        self.mid = channels // reduction

        self.fc1_weights = nn.Parameter(
            torch.zeros(num_experts, self.mid, channels))
        self.fc2_weights = nn.Parameter(
            torch.zeros(num_experts, channels, self.mid))

        self.relu = nn.ReLU(inplace=False)
        self.relu2 = nn.ReLU(inplace=False)
        self._aug_probs = None

    def init_from_channel_attn(self, channel_attn: nn.Module):
        """Initialize all experts from a pretrained ChannelAttention."""
        with torch.no_grad():
            w1 = channel_attn.fc1.weight.squeeze(-1).squeeze(-1)   # (mid, C)
            w2 = channel_attn.fc2.weight.squeeze(-1).squeeze(-1)   # (C, mid)
            for k in range(self.num_experts):
                self.fc1_weights[k].copy_(w1)
                self.fc2_weights[k].copy_(w2)

    def set_aug_probs(self, probs: torch.Tensor):
        """Cache AugClassifier softmax probs (B, K) for current forward."""
        self._aug_probs = probs

    def forward(self, x):
        B, C, H, W = x.shape
        K = self.num_experts

        if self._aug_probs is not None:
            p = self._aug_probs                                   # (B, K)
        else:
            p = x.new_ones(B, K) / K

        W1 = torch.einsum('bk,koi->boi', p, self.fc1_weights)   # (B, mid, C)
        W2 = torch.einsum('bk,koi->boi', p, self.fc2_weights)   # (B, C, mid)

        gap = x.mean((2, 3))                                     # (B, C)

        h = torch.einsum('bc,boc->bo', gap, W1)                  # (B, mid)
        h = self.relu(h)

        attn = torch.einsum('bo,bco->bc', h, W2)                 # (B, C)
...
class SpatialAttention(nn.Module):
    ...
    def __init__(self, kernel_size=3, norm_type=0, channels=768, spatial_group_size=1, var_feature=False):
        super(SpatialAttention, self).__init__()

        if channels % spatial_group_size != 0:
            raise ValueError(
                f'channels ({channels}) must be divisible by spatial_group_size ({spatial_group_size})')

        self.num_groups = spatial_group_size
        self.group_channels = channels // spatial_group_size
        self.var_feature = var_feature
        self.num_feats = 5 if var_feature else 2  # mean,min,max,std,median vs mean,max
        in_channels = self.num_groups * self.num_feats
        self.expansion = self.num_groups * 2
        self.conv = nn.Conv2d(in_channels, self.expansion, kernel_size, stride=1, padding='same', bias=False)
        
        if norm_type == 0:
            self.norm = nn.Identity()
        elif norm_type == 1:
            self.norm = nn.BatchNorm2d(self.expansion)
        elif norm_type == 2:
            self.norm = nn.InstanceNorm2d(self.expansion, affine=True)
        elif norm_type == 3:
            self.norm = nn.InstanceNorm2d(self.expansion, affine=False)
        elif norm_type == 4:
            self.norm = nn.GroupNorm(1, self.expansion)
        else:
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        x_grouped = x.view(B, self.num_groups, self.group_channels, H, W)
        x_mean = x_grouped.mean(dim=2)    # (B, num_groups, H, W)
        x_max = x_grouped.amax(dim=2)     # (B, num_groups, H, W)

        if self.var_feature:
            x_min = x_grouped.amin(dim=2)
            x_std = x_grouped.std(dim=2)
            x_median = x_grouped.median(dim=2).values
            x_attn = torch.cat([x_mean, x_min, x_max, x_std, x_median], dim=1)
        else:
            x_attn = torch.cat([x_mean, x_max], dim=1)

        x_attn = self.norm(self.conv(x_attn))   # (B, num_groups*2, H, W)
        x_attn = x_attn.view(B, self.num_groups, 2, H, W)
        x_attn = F.softmax(x_attn, dim=2)

        positive_mask = x_attn[:, :, 0:1, :, :]  # (B, num_groups, 1, H, W)
        negative_mask = x_attn[:, :, 1:2, :, :]  # (B, num_groups, 1, H, W)

        mask = torch.where(positive_mask >= negative_mask, positive_mask, -negative_mask)
        mask = mask.expand(-1, -1, self.group_channels, -1, -1).reshape(B, C, H, W)

        return x, mask

class SpatialAttention2(nn.Module):
    """N-class spatial attention for domain-aware edge detection.

    Classifies each spatial position into one of `prop_size` classes via
    softmax over channel-mean/max features.  The mask is the argmax class
    probability (strength ∈ [0, 1]).

    Training uses domain CE (source→class 0, target→class N-1) plus an
    Information Maximization loss that shapes the intermediate classes.
    Logits are cached in ``_logits`` for external loss computation.
    """

    def __init__(self, kernel_size=3, channels=768, prop_size=4, var_feature=False):
        super().__init__()
        self.prop_size = prop_size
        self.channels = channels
        self.var_feature = var_feature
        in_channels = 5 if var_feature else 2
        self.conv = nn.Conv2d(in_channels, prop_size, kernel_size, stride=1, padding='same', bias=False)
        self._logits = None

    def forward(self, x):
        B, C, H, W = x.shape
        x_mean = x.mean(dim=1, keepdim=True)    # (B, 1, H, W)
        x_max = x.amax(dim=1, keepdim=True)     # (B, 1, H, W)

        if self.var_feature:
            x_min = x.amin(dim=1, keepdim=True)
            x_std = x.std(dim=1, keepdim=True)
            x_median = x.median(dim=1, keepdim=True).values
            x_attn = torch.cat([x_mean, x_min, x_max, x_std, x_median], dim=1)  # (B, 5, H, W)
        else:
            x_attn = torch.cat([x_mean, x_max], dim=1)  # (B, 2, H, W)

        logits = self.conv(x_attn)               # (B, prop_size, H, W)
        self._logits = logits

        probs = F.softmax(logits, dim=1)          # (B, prop_size, H, W)
        mask, _ = probs.max(dim=1, keepdim=True)  # (B, 1, H, W)
        mask = mask.expand_as(x)                  # (B, C, H, W)

        return x, mask

class SpatialAttention3(nn.Module):
    """Single-channel spatial attention with configurable activation.

    Produces a scalar mask per spatial position.  No auxiliary losses
    (domain CE / IM) are needed — trained with classification CE only.

    Args:
        activation: 'tanh' for [-1, 1] (ResNet), 'sigmoid' for [0, 1] (ViT).
    """

    def __init__(self, kernel_size=3, channels=768, var_feature=False, activation='tanh'):
        super().__init__()
        self.channels = channels
        self.var_feature = var_feature
        self.activation = activation
        in_channels = 5 if var_feature else 2
        self.conv = nn.Conv2d(in_channels, 1, kernel_size, stride=1, padding='same', bias=False)
    ...
    def forward(self, x):
        ...
        if self.activation == 'sigmoid':
            mask = torch.sigmoid(self.conv(x_attn))   # (B, 1, H, W), range [0, 1]
        else:
            mask = torch.tanh(self.conv(x_attn))       # (B, 1, H, W), range [-1, 1]
        mask = mask.expand_as(x)                       # (B, C, H, W)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            use_sam_module: int = -1,
            reverse_se_sam: bool = False,
            parallel_attention: bool = False,
            sam_norm_type: int = 0,
            vit_early_norm_types: Optional[List[int]] = None,
            sam_kernel_size: int = 7,
            spatial_group_size: int = 1,
            vit_last: bool = False,
            vit_closed: Optional[str] = None,
    ) -> None:
        """
        Args:
        ...
        self.vit_last = vit_last

        self.parallel_attention = parallel_attention
        if parallel_attention:
            self.channel_attn = ChannelAttention(channels=embed_dim)
            self.spatial_attn = SpatialAttention(kernel_size=sam_kernel_size, norm_type=sam_norm_type, channels=embed_dim, spatial_group_size=spatial_group_size)

        self.vit_closed = vit_closed
        if vit_closed is not None:
            if vit_closed not in ('same', 'diff'):
                raise ValueError(f"vit_closed must be 'same' or 'diff', got '{vit_closed}'")
            if not self.parallel_attention:
                raise ValueError('--vit-closed requires --parallel-attention')
            if vit_closed == 'diff':
                self.channel_attn_last = ChannelAttention(channels=embed_dim)
                self.spatial_attn_last = SpatialAttention(kernel_size=sam_kernel_size, norm_type=sam_norm_type, channels=embed_dim, spatial_group_size=spatial_group_size)

        if vit_early_norm_types is None:
            vit_early_norm_types = [0, 0, 0, 0]
        if len(vit_early_norm_types) != 4:
            raise ValueError(f'vit_early_norm_types must have exactly 4 integers, got {vit_early_norm_types}')
        for i, norm_type in enumerate(vit_early_norm_types):
            if norm_type not in (0, 1, 2, 3, 4):
                raise ValueError(f'Invalid vit_early_norm_types[{i}]={norm_type}. Allowed values: 0,1,2,3,4.')
        self.vit_early_norm_types = list(vit_early_norm_types)
        self.vit_early_norms = nn.ModuleList([
            self._create_token_norm(norm_type, embed_dim) for norm_type in self.vit_early_norm_types
        ])
    ...
    def _apply_parallel_attention(self, x: torch.Tensor) -> torch.Tensor:
        """Apply Channel and Spatial attention in parallel: mask * attn * x."""
        attn = self.channel_attn(x)                                       # (B, C, 1, 1)
        sa_input = x.detach() if getattr(self, 'attn_detach', False) else x
        _, mask = self.spatial_attn(sa_input)                             # (B, C, H, W)
        attended = mask * attn * x
        if getattr(self, 'attn_residual', False):
            return x + attended
        return attended

    def _apply_parallel_attention_last(self, x: torch.Tensor) -> torch.Tensor:
        """Apply last-position parallel attention (same or diff modules depending on vit_closed)."""
        if self.vit_closed == 'same':
            return self._apply_parallel_attention(x)
        ca = self.channel_attn_last(x)
        sa_input = x.detach() if getattr(self, 'attn_detach', False) else x
        _, mask = self.spatial_attn_last(sa_input)
        attended = mask * ca * x
        if getattr(self, 'attn_residual', False):
            return x + attended
        return attended
    ...
    @staticmethod
    def _create_token_norm(norm_type: int, embed_dim: int) -> nn.Module:
        if norm_type == 0:
            return nn.Identity()
        if norm_type == 1:
            return nn.BatchNorm1d(embed_dim, affine=True)
        if norm_type == 2:
            return nn.BatchNorm1d(embed_dim, affine=False)
        if norm_type == 3:
            return nn.InstanceNorm1d(embed_dim, affine=True)
        if norm_type == 4:
            return nn.InstanceNorm1d(embed_dim, affine=False)
        raise ValueError(f'Invalid norm_type: {norm_type}')

    def _apply_early_token_norm(self, x: torch.Tensor, norm_idx: int) -> torch.Tensor:
        norm = self.vit_early_norms[norm_idx]
    ...
    def forward_features(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        apply_early = self.parallel_attention and (not self.vit_last or self.vit_closed is not None)
        apply_last = self.parallel_attention and (self.vit_last or self.vit_closed is not None)

        if apply_early:
            raw_input = x
            x = self.patch_embed.proj(x)

            if getattr(self, 'aug_classifier', None) is not None:
                with torch.no_grad():
                    self.aug_classifier.eval()
                    aug_logits = self.aug_classifier(raw_input)
                s4 = self.aug_classifier.get_stage4_feature()     # (B, 256, 14, 14)
                x = x + s4.mean(dim=1, keepdim=True)             # (B, 1, 14, 14) broadcast

                if hasattr(self.channel_attn, 'set_aug_probs'):
                    aug_probs = F.softmax(aug_logits, dim=1)
                    self.channel_attn.set_aug_probs(aug_probs)

            x = self._apply_parallel_attention(x)
        ...
        x = self.patch_drop(x)
        x = self.norm_pre(x)
        x = self._apply_early_token_norm(x, 0)

        if attn_mask is not None:
            for i, blk in enumerate(self.blocks):
                x = blk(x, attn_mask=attn_mask)
                if i < 3:
                    x = self._apply_early_token_norm(x, i + 1)
        elif self.grad_checkpointing and not torch.jit.is_scripting():
            for i, blk in enumerate(self.blocks):
                x = checkpoint(blk, x)
                if i < 3:
        ...
        if apply_last:
            ...
            if self.vit_closed is not None:
                patch_tokens = self._apply_parallel_attention_last(patch_tokens)
            else:
                # print('DEBUG Checkpoint 1')
                patch_tokens = self._apply_parallel_attention(patch_tokens)
    ...
    def get_nll_loss(self) -> None:
        return None

    def get_nll_sam_loss(self) -> None:
        return None

    def get_spatial_logits(self):
        """Return list of cached SpatialAttention logits (before softmax) from the last forward pass."""
        logits_list = []
        if self.parallel_attention:
            logits = getattr(self.spatial_attn, '_logits', None)
            if logits is not None:
                logits_list.append(logits)
            if self.vit_closed == 'diff':
                logits = getattr(self.spatial_attn_last, '_logits', None)
                if logits is not None:
                    logits_list.append(logits)
        return logits_list if logits_list else None

    def get_last_masks(self):
        """Return list of cached SpatialAttention masks from the last forward pass."""
        masks = []
        if self.parallel_attention:
            m = getattr(self.spatial_attn, '_last_mask', None)
            if m is not None:
                masks.append(m)
            if self.vit_closed == 'diff':
                m = getattr(self.spatial_attn_last, '_last_mask', None)
                if m is not None:
                    masks.append(m)
...
def _create_vision_transformer(
    ...
    if 'siglip' in variant and kwargs.get('global_pool', None) != 'map':
        strict = False
    if kwargs.get('parallel_attention', False) or kwargs.get('vit_closed', None) is not None:
        strict = False
