...
class ByobNet(nn.Module):
    ...
    def _apply_parallel_attention(self, x: torch.Tensor) -> torch.Tensor:
        """Apply optional SOA attention to an NCHW feature map."""
        attn = self.channel_attn(x)
        sa_input = x.detach() if getattr(self, 'attn_detach', False) else x
        _, mask = self.spatial_attn(sa_input)
        attended = mask * attn * x
        if getattr(self, 'attn_residual', False):
            return x + attended
        return attended

    def _apply_stem_attention(self, x: torch.Tensor, raw_input: torch.Tensor) -> torch.Tensor:
        if not getattr(self, 'parallel_attention', False):
            return x
        ...
        if getattr(self, 'aug_classifier', None) is not None:
            ...
            s4 = self.aug_classifier.get_stage4_feature()
            residual = s4.mean(dim=1, keepdim=True)
    ...
    def forward_intermediates(
        ...
        x = self._apply_stem_attention(x, raw_input)
