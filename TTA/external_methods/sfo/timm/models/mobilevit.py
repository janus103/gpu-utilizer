...
from .vision_transformer import Block as TransformerBlock, ChannelAttention, SpatialAttention
...
def _pop_soa_kwargs(kwargs):
    return {k: kwargs.pop(k) for k in list(kwargs.keys()) if k in _SOA_KWARGS}


def _stem_channels(cfg: ByoModelCfg) -> int:
    stem_chs = cfg.stem_chs
    if isinstance(stem_chs, (list, tuple)):
        stem_chs = stem_chs[-1]
    stem_chs = stem_chs or cfg.blocks[0].c
    return int(round(stem_chs * cfg.width_factor))


def _install_mobilevit_soa(model: ByobNet, cfg: ByoModelCfg, soa_kwargs):
    if not soa_kwargs.get('parallel_attention', False):
        return model

    channels = _stem_channels(cfg)
    param = next(model.parameters())
    dd = {'device': param.device, 'dtype': param.dtype}
    model.parallel_attention = True
    model.channel_attn = ChannelAttention(channels=channels).to(**dd)
    model.spatial_attn = SpatialAttention(
        kernel_size=soa_kwargs.get('sam_kernel_size', 2),
        norm_type=soa_kwargs.get('sam_norm_type', 0),
        channels=channels,
        spatial_group_size=soa_kwargs.get('spatial_group_size', 1),
    ).to(**dd)
    return model


def _create_mobilevit(variant, cfg_variant=None, pretrained=False, **kwargs):
    soa_kwargs = _pop_soa_kwargs(kwargs)
    ...
    model = build_model_with_cfg(
        ...
        **kwargs)
    return _install_mobilevit_soa(model, cfg, soa_kwargs)
