...
Adds SEModule (Channel Attention) and SAModule (Spatial Attention) with Gaussian loss
for TTA pre-training. Supports both ResNet and ViT architectures.

- ResNet: SE/SAM placed after conv1 in forward_features (4D NCHW).
- ViT: SE/SAM placed after patch_embed.proj, before flatten to NLC sequence.

The Gaussian NLL loss is retrieved via model.get_nll_loss() / model.get_nll_sam_loss().
"""
import argparse
import copy
import importlib
...
group.add_argument('--val-calibrate', action='store_true', default=False,
                   help='Run auxiliary-only calibration on validation loader before main training')
...
group.add_argument('--half-mode', action='store_true', default=False,
                   help='Half-mode: pair original and color-augmented images sharing geometric transforms. '
                        'Original half gets CE auxiliary loss on SpatialAttention logits to enforce mask=1.0.')
group.add_argument('--half-mode-loss-weight', type=float, default=1.0,
                   help='Weight for the half-mode auxiliary CE loss (default: 1.0)')
...
def _get_base_model(model):
    """Unwrap DDP/compiled model to access get_nll_loss() / get_nll_sam_loss()."""
    m = model
    if hasattr(m, 'module'):
        m = m.module
...
def _set_requires_grad_state(model: nn.Module, requires_grad_flags):
    """Restore requires_grad flags in model parameter order."""
    for param, requires_grad in zip(model.parameters(), requires_grad_flags):
        param.requires_grad = requires_grad


class HalfModeDataset(torch.utils.data.Dataset):
    """Produces (original, augmented, target) triples sharing geometric transforms.

    Geometric AA ops are applied to both copies identically;
    color AA ops are applied only to the augmented copy.
    """

    def __init__(self, dataset, primary_transform, aa_policy, final_transform):
    ...
    def __getitem__(self, index):
        ...
        return self.final_transform(img), self.final_transform(aug_img), target

    def __len__(self):
        return len(self.dataset)


def half_mode_collate_fn(batch):
    """Collate (original, augmented, target) triples into a single batch.

    Returns (input, target) where input[0:B/2] is original, input[B/2:B] is augmented,
    and target is duplicated accordingly.
    """
    origs, augs, targets = zip(*batch)
    origs = torch.stack(origs)
    augs = torch.stack(augs)
    if isinstance(targets[0], torch.Tensor):
        targets = torch.stack(targets)
    else:
        targets = torch.tensor(targets, dtype=torch.long)
    return torch.cat([origs, augs], dim=0), torch.cat([targets, targets], dim=0)
...
def main():
    ...
    if args.half_mode:
        if args.batch_size % 2 != 0:
            raise ValueError(f'--half-mode requires even batch_size, got {args.batch_size}')
        mixup_active = args.mixup > 0 or args.cutmix > 0. or args.cutmix_minmax is not None
        if mixup_active:
            raise ValueError('--half-mode is incompatible with mixup/cutmix')
        if not args.aa:
            raise ValueError('--half-mode requires --aa (e.g. --aa v0)')
    ...
    if args.half_mode:
        ...
        aa_hparams = dict(
            ...
            img_mean=tuple([min(255, round(255 * x)) for x in data_config['mean']]),
        )
        if train_interpolation and train_interpolation != 'random':
            from timm.data.transforms import str_to_pil_interp
            aa_hparams['interpolation'] = str_to_pil_interp(train_interpolation)
        aa_policy_name = args.aa.split('-')[0]
        aa_policy = auto_augment_policy(aa_policy_name, hparams=aa_hparams)

        dataset_train = HalfModeDataset(dataset_train, primary_tf, aa_policy, final_tf)
        ...
        loader_train = torch.utils.data.DataLoader(
            ...
            num_workers=args.workers,
            sampler=sampler,
            collate_fn=half_mode_collate_fn,
            pin_memory=args.pin_mem,
            drop_last=True,
        )
        ...
        _logger.info(
            f'Half-mode enabled: DataLoader batch_size={half_batch} (x2 via collate = {args.batch_size}), '
            f'AA policy={aa_policy_name}, num_sub_policies={len(aa_policy)}'
        )
    else:
        collate_fn = None
        mixup_active = args.mixup > 0 or args.cutmix > 0. or args.cutmix_minmax is not None
    ...
    if args.val_calibrate:
        if loader_eval is None:
            raise ValueError('--val-calibrate requires validation loader. Set --val-split.')
        calibration_epochs = 10
        prev_requires_grad_flags = [param.requires_grad for param in model.parameters()]
        calibration_trainable = _apply_train_mode(model, 2)
        if utils.is_primary(args):
            _logger.info(
                f'Starting validation calibration ({calibration_epochs} epochs) with auxiliary-only training. '
                f'trainable params: {calibration_trainable}'
            )
        for cal_epoch in range(calibration_epochs):
            if args.distributed and hasattr(loader_eval.sampler, 'set_epoch'):
                loader_eval.sampler.set_epoch(cal_epoch)
            train_one_epoch(
                epoch=cal_epoch,
                model=model,
                loader=loader_eval,
        ...
        _set_requires_grad_state(model, prev_requires_grad_flags)
        restored_trainable = sum(param.numel() for param in model.parameters() if param.requires_grad)
        if utils.is_primary(args):
            _logger.info(
                f'Validation calibration finished. Restored trainable params for main training: {restored_trainable}'
            )
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        def _forward():
            ...
            if accum_steps > 1:
                _loss /= accum_steps
            return _loss, _cls_loss, _gaussian_loss, _half_mode_loss
        ...
        if has_no_sync and not need_update:
            with model.no_sync():
                loss, cls_loss, gaussian_loss, half_mode_loss = _forward()
                _backward(loss)
        else:
            loss, cls_loss, gaussian_loss, half_mode_loss = _forward()
            _backward(loss)
        ...
        if update_idx % args.log_interval == 0 or last_batch:
            ...
            if utils.is_primary(args):
                hm_str = ''
                if args.half_mode:
                    hm_str = f'  HM-Loss: {half_mode_losses_m.val:#.3g} ({half_mode_losses_m.avg:#.3g})'
                _logger.info(
