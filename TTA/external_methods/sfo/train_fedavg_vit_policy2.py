#!/usr/bin/env python3
"""FedAvg Training — ViT-B with Parallel Channel/Spatial Attention (SA3) + AA Policy Clients

Federated Averaging over 15 ImageNet-C corruption clients PLUS N AutoAugment
policy clients for Vision Transformer (ViT-B) models.
...
_logger = logging.getLogger('fedavg_vit_policy2')
...
def _get_attention_channels(model: nn.Module) -> int:
...
def _is_head_param(param_name: str) -> bool:
    name = param_name.lower()
    return name.startswith(('head.', 'fc_norm.', 'attn_pool.'))


def _is_norm_param(param_name: str) -> bool:
    """Match normalization params inside transformer or MobileViT blocks."""
    name = param_name.lower()
    if 'blocks.' in name and ('.norm' in name or '.ls' in name):
        return True
    if name == 'norm.weight' or name == 'norm.bias':
        return True
    if name.startswith('stages.') and any(
            kw in name for kw in ('.bn', '.norm', '.norm1', '.norm2', '.norm3')):
        return True
    return False
...
parser = argparse.ArgumentParser(description='FedAvg — ViT Parallel Attention (SA3) Training')
...
group.add_argument('--resume-aug', type=str, default=None,
                   help='Load frozen AugClassifier checkpoint; stage4 mean residual '
                        '(256→mean→1, 14x14) added to patch_embed, logits for MoEChannelAttention')
...
group.add_argument('--vit-last', action='store_true', default=False,
                   help='Apply attention after all ViT blocks instead of at patch embed')
group.add_argument('--vit-closed', type=str, default=None, choices=['same', 'diff'],
                   help='Apply attention at both early and last positions')
...
group.add_argument('--output-dir', type=str, default=None,
                   help='Output directory (default: ./output/fedavg_vit_sa3_k{vit_kernel_size}_tm{train_mode})')
...
def main():
    ...
    if args.output_dir is None:
        ...
        model_prefix = f'fedavg_{safe_model_name(args.model)}' if is_mobilevit else 'fedavg_vit'
        args.output_dir = (f'./output/{model_prefix}_sa3_k{args.vit_kernel_size}'
    ...
    if not is_mobilevit:
        if args.vit_last:
            soa_kwargs['vit_last'] = True
        if args.vit_closed is not None:
            soa_kwargs['vit_closed'] = args.vit_closed
        if args.vit_early_norm_types is not None:
            soa_kwargs['vit_early_norm_types'] = args.vit_early_norm_types
    elif args.vit_last or args.vit_closed is not None or args.vit_early_norm_types is not None:
        raise ValueError('vit_last/vit_closed/vit_early_norm_types are ViT-only options.')
    ...
    _logger.info(f'SpatialAttention3 installed at {attn_site}: channels={ch}, '
                 ...
                 f'activation=sigmoid [0,1]')

    # ── Attention fusion options ──
    if args.parallel_attention:
        base = _get_base_model(model)
        base.attn_detach = getattr(args, 'detach', False)
        base.attn_residual = getattr(args, 'residual', False)
        _logger.info(f'Attention fusion: detach={base.attn_detach}, residual={base.attn_residual}')

    # ── Load frozen AugClassifier (stage4 mean residual + logits for MoE) ──
    ...
    if args.resume_aug:
        ...
        if is_mobilevit:
            _logger.info('  Stage4 mean residual: mean → resize/broadcast to MobileViT stem feature')
        else:
            _logger.info(f'  Stage4 mean residual: (256,14,14) → mean → (1,14,14) broadcast to patch_embed')
