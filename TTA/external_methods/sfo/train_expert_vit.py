#!/usr/bin/env python3
"""Expert Patch-Embed + LayerNorm Training Script for ViT.

This script trains the patch embedding layer and all LayerNorm layers
of a Vision Transformer (ViT) for a SINGLE augmentation type.
...
Usage:
    python train_expert_vit.py --data-dir /path/to/imagenet --model vit_base_patch16_224 \
        --pretrained --aug-type 0 --sl-max 0.5 --epochs 100
"""
import argparse
import datetime
import importlib
...
_logger = logging.getLogger('train_expert_vit')
...
group.add_argument('--train-mode', type=str, default='patch-embed-all-ln',
                   choices=['patch-embed-all-ln'],
                   help='Training mode: '
                        '"patch-embed-all-ln" = patch embedding (conv proj) + all LayerNorm layers')
...
def main():
    ...
    if args.train_mode == 'patch-embed-all-ln':
        # patch_embed (conv projection) + all LayerNorm layers
        _logger.info('Training mode: patch-embed-all-ln (patch_embed + all LayerNorm)')
        
        # 1) Unfreeze patch_embed
        patch_embed_params = 0
        if hasattr(model, 'patch_embed'):
            for param in model.patch_embed.parameters():
                param.requires_grad = True
                trainable_params.append(param)
                patch_embed_params += param.numel()
            _logger.info(f'  patch_embed: {patch_embed_params} params')
        else:
            _logger.warning('  Model has no patch_embed attribute!')
        
        # 2) Unfreeze all LayerNorm layers
        ln_count = 0
        ln_params = 0
        for name, module in model.named_modules():
            if isinstance(module, nn.LayerNorm):
                ...
                ln_count += 1
        _logger.info(f'  Total LayerNorm layers: {ln_count}')
        _logger.info(f'  LayerNorm params: {ln_params}')
    ...
    if args.resume:
        ...
        # Load all saved trainable params (patch_embed + LayerNorm weights)
        loaded_keys = []
        for key in src:
        ...
        _logger.info(f'Loaded {len(loaded_keys)} keys from checkpoint')
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if (epoch + 1) % args.val_interval == 0 or epoch == num_epochs - 1:
                ...
                if utils.is_primary(args):
                    # Save trainable parameters (patch_embed + all LayerNorm)
                    # LayerNorm has no running stats, so we only save requires_grad params
                    trained_state = {}
...
def train_one_epoch(epoch, model, loader, optimizer, criterion, args, device, amp_autocast=suppress, loss_scaler=None):
    ...
    # Use eval mode for the entire model (ViT has no BatchNorm).
    # This disables dropout in frozen transformer blocks, ensuring
    # deterministic features from the pretrained backbone.
    # LayerNorm behaves identically in train/eval (no running stats).
    model.eval()
...
def validate(model, loader, criterion, args, device, amp_autocast=suppress):
    ...
    # Set model to eval mode (matches training behavior)
    model.eval()
