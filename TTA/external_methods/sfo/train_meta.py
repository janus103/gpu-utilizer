#!/usr/bin/env python3
"""Meta-Learning Training Script for TTA-Aware Aug Classifier (Phase 2).

Meta-learns the aug_classifier so that its gradient signal, when used to
adapt the stem (conv1) at test time, improves classification accuracy.

Architecture:
    - Stem: conv1 ONLY (no bn1, no act1) — the TTA adaptation target.
    - Aug Classifier: MLP that provides TTA gradient signal (meta-learned).
    - Backbone + FC: frozen pretrained model for classification evaluation.

Training (MAML-style):
    Inner loop: simulate TTA — adapt conv1 using entropy-max loss via aug_classifier.
    Outer loop: evaluate classification with adapted conv1 → meta-update aug_classifier.

Usage:
    python train_meta.py --data-dir /path/to/imagenet --model resnet50 --pretrained \\
...
def meta_inner_loop_simple(
    ...
    optimizer = torch.optim.SGD([model.conv1.weight], lr=inner_lr, momentum=0.9)
...
def meta_step_differentiable(
    ...
    """One full differentiable meta-step: inner loop + outer loss.

    This is the core of Phase 2 meta-learning.

    Flow:
      1. Inner loop: adapt conv1 (K steps, graph preserved).
      2. Outer forward: adapted conv1 → bn1 → backbone → fc → L_outer.
      3. Return L_outer (calling ``.backward()`` sends meta-gradient to aug_classifier).

    Args:
        images: Input batch ``[B, 3, H, W]``.
        class_labels: Ground-truth class labels ``[B]``.
        model: Pretrained model (frozen backbone).
        aug_classifier: Aug classifier (meta-update target).
        fsc_centroids: FSC centroids ``[num_classes, 1024]``.
        fsc_diff_mode: ``'subtract'`` or ``'orthogonal'``.
        inner_lr: Inner loop learning rate (alpha).
        inner_steps: Inner loop steps (K).
        inner_alpha: Weight for entropy maximization term (default: 1.0).
        inner_beta: Weight for repel-corrupted term (default: 0.1).
        inner_gamma: Weight for attract-centroid term (default: 0.5).

    Returns:
        L_outer: Classification loss (scalar, in graph — call ``.backward()``).
        info: Dict with ``top1``, ``top5``, ``loss`` for logging.
    """
    model.eval()
    ...
    # ---- Outer loss ----
    L_outer = F.cross_entropy(logits, class_labels)
...
def main():
    ...
    # ==========================================================================
    _logger.info(
        f'Starting meta-learning for {num_epochs} epochs  '
        f'(inner_lr={args.inner_lr}, inner_steps={args.inner_steps}, '
...
def train_one_epoch_meta(
    ...
    For each batch:
      1. Inner loop: adapt conv1 via entropy-max (differentiable, K steps).
      2. Outer loop: classification loss with adapted conv1 → meta-update aug_classifier.
      3. Reset conv1 (it was never modified in-place — virtual params only).
