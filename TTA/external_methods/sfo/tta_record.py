#!/usr/bin/env python3
"""TTA Recording: Run TTA for classes 0-999 and save per-epoch metrics to TSV.
...
Usage:
    python tta_record.py \\
        --data-dir /home/oem/jin/datasets/imagenet-c \\
        --val-split pixelate/5 \\
        --backbone-checkpoint ./ZOA_WEIGHT/ZOA_resnet50_timm_format.pth \\
        --aug-classifier-checkpoint ./ZOA_FSC/phase1_resnet50_direct_warmrestart/best.pth.tar \\
        --target-aug-logits-file ./output/eval_analyze_per_class.tsv \\
        --tta-norm l2 --epochs 20 \\
        --output-tsv ./output/tta_record.tsv

    # Or use direct 8 values (same target for all classes):
    python tta_record.py ... --target-aug-logits "5.46,-3.67,3.05,3.14,-9.32,-11.48,-1.54,-11.04"
"""
import argparse
import csv
import logging
...
_logger = logging.getLogger('tta_record')
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description='TTA Record: Run TTA for classes 0-999, save per-epoch metrics to TSV'
    )
    ...
    parser.add_argument(
        ...
        help='Path to phase-1 aug classifier checkpoint.',
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Output TSV path. Default: output/tta_record_{val_safe}_{norm}_{epochs}.tsv',
    )
...
def main():
    ...
    if not args.output_tsv:
        args.output_tsv = (
            f'./output_tta_record_mean/tta_record_{val_safe}_{args.tta_norm}_ep{args.epochs}.tsv'
        )
    ...
    # -------------------------------------------------------------------------
    # Load backbone and aug classifier once
    # -------------------------------------------------------------------------
    ...
    # Save initial conv1 state for reset between classes
    conv1_state = {
        k: v.cpu().clone() for k, v in backbone.conv1.state_dict().items()
    }
    ...
    for target_class in range(args.start_class, args.end_class):
        ...
        # Reset stem to initial state
        backbone.conv1.load_state_dict(
            {k: v.to(device) for k, v in conv1_state.items()}
        )
