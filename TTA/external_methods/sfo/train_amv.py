...
class AmvLearningWrapper(nn.Module):
    def __init__(self, model, stem_ae: StemAutoEncoder, amv: nn.Parameter, aug_name: str, loss_type: str = 'error', layer_mode: str = 'conv'):
        super().__init__()
        self.model = model
        self.stem_ae = stem_ae
        self.amv = amv
        ...
        # Initialize Base Latent
        with torch.no_grad():
            weight = self.stem.weight.detach()
            means = weight.mean(dim=(2, 3))
            vars_ = weight.var(dim=(2, 3), unbiased=False)
            flat_stats = torch.cat([means.flatten(), vars_.flatten()], dim=0).unsqueeze(0)
            if next(self.stem_ae.parameters()).is_cuda:
                flat_stats = flat_stats.to(next(self.stem_ae.parameters()).device)
            _, z_base = self.stem_ae(flat_stats)
            self.register_buffer('z_base', z_base)
            self.register_buffer('weight_stats_mean', means)
            self.register_buffer('weight_stats_std', vars_.sqrt() + 1e-6)
            self.register_buffer('base_weight', weight.clone())

    def _generate_weight(self, latent_vector):
        stats = self.stem_ae.decoder(latent_vector)
        split_idx = self.out_channels * self.in_channels
        pred_means = stats[:, :split_idx].view(-1, self.out_channels, self.in_channels)
        raw_vars = stats[:, split_idx:].view(-1, self.out_channels, self.in_channels)
        pred_vars = torch.nn.functional.softplus(raw_vars)
        pred_stds = (pred_vars + 1e-6).sqrt()
    ...
    def forward_stem(self, x, weights):
        B = x.shape[0]
        x_g = x.reshape(1, B * self.in_channels, x.shape[-2], x.shape[-1])
        w_g = weights.reshape(B * self.out_channels, self.in_channels, self.kernel_size[0], self.kernel_size[1])
    ...
    def forward(self, x):
        # x: [Aug, Clean, Aug, Clean...]
        aug_imgs = x[0::2]
        clean_imgs = x[1::2]
        
        z_combined = self.z_base + self.amv # [1, L]
        
        # 1. Clean Image -> Modified Stem
        re_weights = self._generate_weight(z_combined)
        re_weights = re_weights.repeat(clean_imgs.shape[0], 1, 1, 1, 1)
        ofm_re = self.forward_stem(clean_imgs, re_weights)
        
        # 2. Aug Image -> Original Stem
        ori_weights = self.base_weight.unsqueeze(0).repeat(aug_imgs.shape[0], 1, 1, 1, 1)
        ofm_aug = self.forward_stem(aug_imgs, ori_weights)
        
        if self.layer_mode == 'bn_relu':
            if hasattr(self.model, 'bn1'):
                ofm_re = self.model.bn1(ofm_re)
                ofm_aug = self.model.bn1(ofm_aug)
            if hasattr(self.model, 'act1'):
                ofm_re = self.model.act1(ofm_re)
                ofm_aug = self.model.act1(ofm_aug)
            elif hasattr(self.model, 'relu'):
                ofm_re = self.model.relu(ofm_re)
                ofm_aug = self.model.relu(ofm_aug)
                
        return ofm_re, ofm_aug
...
group.add_argument('--amv-mode', action='store_true', help='Enable AMV training mode')
group.add_argument('--aug-name', type=str, default='Rotate', help='Augmentation name for AMV training')
group.add_argument('--aug-severity', type=float, default=0.5, help='Severity for AMV augmentation')
group.add_argument('--amv-loss', type=str, default='error', choices=['error', 'distill'], help='Loss type')
group.add_argument('--amv-layer', type=str, default='conv', choices=['conv', 'bn_relu'], help='Layer to match OFM')
...
def main():
    ...
    if args.amv_mode:
        aug_manager = AugmentationManager(sensitivity=args.aug_severity, included_augs=[args.aug_name])
        # n_aug_mode must be False for standard path, but we are hijacking it via amv_mode
        n_aug_mode = False 
    else:
    ...
    if (n_aug_mode or args.amv_mode) and args.naflex_loader:
        parser.error('--n-aug is not supported with --naflex-loader')
    if (n_aug_mode or args.amv_mode) and args.aug_splits > 1:
        parser.error('--n-aug cannot be combined with --aug-splits/JS D mode')
    if (n_aug_mode or args.amv_mode) and args.prefetcher:
        if utils.is_primary(args):
            _logger.warning('Disabling prefetcher for --n-aug mode / amv-mode (custom collate required).')
    ...
    if args.amv_mode and args.latent_size is None:
         args.latent_size = 128 # Default if not set, to avoid None usage in filename
    ...
    if args.amv_mode:
        ...
        if stem_ae is None:
            ...
            stem_dim = stem.out_channels * stem.in_channels
            if args.latent_size is None:
                 args.latent_size = 128
            stem_ae = StemAutoEncoder(input_dim=2 * stem_dim, latent_dim=args.latent_size)
            if args.ae_checkpoint:
                checkpoint = torch.load(args.ae_checkpoint, map_location='cpu')
                stem_ae.load_state_dict(checkpoint['state_dict'] if 'state_dict' in checkpoint else checkpoint)
        
        stem_ae.eval()
        for p in stem_ae.parameters():
            p.requires_grad = False
            
        amv = nn.Parameter(torch.zeros(1, args.latent_size))
        
        model = AmvLearningWrapper(
            model, 
            stem_ae, 
            amv, 
            aug_name=args.aug_name, 
            loss_type=args.amv_loss, 
            layer_mode=args.amv_layer
        )
        ...
        if utils.is_primary(args):
            _logger.info(f'Wrapped model with AmvLearningWrapper. Training AMV only.')
    ...
    if args.naflex_loader:
        ...
        # setup mixup / cutmix
        if n_aug_mode or args.amv_mode:
    ...
    loader_eval = None
    if args.val_split and not args.amv_mode:
        ...
        if args.naflex_loader:
            ...
            # Use PerSampleAugmentDataset also for validation to generate (Aug, Clean) pairs
            # Re-use the same transforms and aug_manager as training
            # Note: pre_transform includes RandomResizedCrop/Flip which is random.
            # For validation we usually want deterministic.
            # But AMV task is to mimic Augmentation. If we use deterministic CenterCrop, 
            # we should also apply Augmentation to it.
            # So we use the same logic as training but maybe with CenterCrop?
            # Or just keep it random to validate generalization over crops?
            # User requirement implies evaluating the task of mimicking.
            
            # Let's use CenterCrop for geometric stability, then apply Augmentation.
            # But PerSampleAugmentDataset expects pre_transform.
            
            # Construct validation transforms
            val_input_size = data_config['input_size'][-2:]
            if data_config['crop_pct']:
                scale_size = int(math.floor(val_input_size[0] / data_config['crop_pct']))
            else:
                scale_size = int(math.floor(val_input_size[0] / 0.875))
            
            # Standard Val Pre-transform (Resize + CenterCrop)
            ...
            # Post-transform same as train
            val_post_transform = transforms.Compose([
                transforms.ToTensor(),
                transforms.Normalize(mean=data_config['mean'], std=data_config['std'])
            ])
            
            dataset_eval_amv = PerSampleAugmentDataset(
                dataset_eval, 
                aug_manager=aug_manager, # Same manager (single fixed aug)
            ...
            loader_eval = create_loader(
                dataset_eval_amv,
                input_size=data_config['input_size'],
                collate_fn=collate_per_sample_aug, # MUST use collate
                **common_loader_kwargs,
                **eval_loader_kwargs,
            )
    ...
    if args.amv_mode:
        eval_metric = 'loss' # Force loss metric for AMV
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.amv_mode:
                # In AMV mode, we skip validation and use training loss as metric
                if utils.is_primary(args):
                    _logger.info("Skipping validation in AMV mode (using training loss as metric)")
                eval_metrics = None
            elif loader_eval is not None:
            ...
            if eval_metrics is not None:
                ...
                # If no eval metrics (e.g. AMV mode), use training loss
                if args.amv_mode:
            ...
            if saver is not None:
                ...
                if args.amv_mode and utils.is_primary(args):
                     amv_filename = f"{args.model}_{args.aug_name}_SEV_{args.aug_severity}_AE_{args.latent_size}_.pth"
                     amv_path = os.path.join(output_dir, amv_filename)
                     if hasattr(model, 'module'):
                         amv_tensor = model.module.amv.data
                     else:
                         amv_tensor = model.amv.data
                     torch.save({'amv': amv_tensor}, amv_path)
                     _logger.info(f"Saved AMV parameter to {amv_path}")
    ...
    for batch_idx, batch_data in enumerate(loader):
        ...
        def _forward():
            with amp_autocast():
                ...
                if args.amv_mode:
                    ofm_re, ofm_aug = model(input)
                    
                    if args.amv_loss == 'error':
                        amv_loss = torch.nn.functional.mse_loss(ofm_re, ofm_aug)
                    elif args.amv_loss == 'distill':
                        # Access inner model to run backbone
                        inner_model = model.module.model if hasattr(model, 'module') else model.model
                        stem = model.module.stem if hasattr(model, 'module') else model.stem
                        
                        # Monkey patch stem forward
                        original_forward = stem.forward
                        
                        # Forward RE (Clean -> RE_CONV -> Backbone)
                        stem.forward = lambda x: ofm_re
                        # We pass dummy input of correct batch size (B/2)
                        # input is [B, C, H, W]. clean_imgs is B/2.
                        clean_imgs = input[1::2]
                        logits_re = inner_model(clean_imgs)
                        
                        # Forward AUG (Aug -> ORI_CONV -> Backbone)
                        stem.forward = lambda x: ofm_aug
                        aug_imgs = input[0::2]
                        logits_aug = inner_model(aug_imgs)
                        
                        stem.forward = original_forward
                        
                        # KL Div (Match RE to AUG)
                        # We want RE (Student) to match AUG (Teacher)
                        log_probs_re = torch.log_softmax(logits_re, dim=-1)
                        probs_aug = torch.softmax(logits_aug, dim=-1)
                        amv_loss = torch.nn.functional.kl_div(log_probs_re, probs_aug, reduction='batchmean')
...
def validate(
    ...
    with torch.inference_mode():
        for batch_idx, batch_data in enumerate(loader):
            ...
            with amp_autocast():
                # In AMV mode, validation should also compute AMV loss
                if args.amv_mode:
                     # For validation, we need to manually create pairs if the loader doesn't provide them
                     # But standard validation loader provides single images.
                     # We can simulate AMV loss by creating Aug version on the fly or just rely on train loss.
                     # BUT user asked to use validation loss as metric.
                     # So we MUST perform AMV forward here.
                     
                     # We need to construct input pairs [Aug, Clean] from the batch
                     # input: [B, C, H, W] -> Treat these as "Clean" images
                     # We need to generate "Aug" images using the SAME augmentation policy as training.
                     
                     # Access Augmentation Manager from args/global context? 
                     # It's not passed to validate.
                     # However, AmvLearningWrapper expects input [Aug, Clean, Aug, Clean...]
                     
                     # Let's create Aug version.
                     # We need the transform.
                     # Re-create simple transform here or pass it?
                     # Passing is cleaner but requires signature change.
                     # Let's assume input is Clean.
                     
                     # We need to apply the specific augmentation: args.aug_name with args.aug_severity
                     # This requires cpu processing (PIL) usually, but input is Tensor on GPU.
                     # Ideally we should use a Validation Dataset that also produces Pairs if AMV mode is on.
                     pass

                output = model(input)
                if not args.amv_mode and isinstance(output, (tuple, list)):
                    # If model returns tuple in eval mode (it shouldn't if implemented correctly in forward), pick first
                    # BUT in AMV mode, we expect a tuple (ofm_re, ofm_aug), so we must NOT unpack it here.
                    output = output[0]
                
                # In AMV Mode, output is likely (ofm_re, ofm_aug) tuple if we passed correct input structure.
                # But here input is just [B, C, H, W] (Clean).
                # AmvLearningWrapper.forward expects interleaved [Aug, Clean].
                # If we just pass Clean, it will fail or produce garbage.
                
                # CRITICAL FIX: Validation Loader for AMV Mode must also be PerSampleAugmentDataset-like.
                # But we didn't change validation loader creation in main().
                
                # Let's handle this in main() first to ensure loader is correct.
                # If loader is correct, input will be correct structure.
                
                if args.amv_mode:
                    # loss calculation is handled in the AMV block below
                    loss = torch.tensor(0.0, device=device)
                else:
                    loss = loss_fn(output, target) # This will fail for AMV tuple output
            
            # AMV Mode Handling
            if args.amv_mode:
                 # If we fixed loader, output is (ofm_re, ofm_aug)
                 # We need to compute AMV loss manually here as loss_fn is CrossEntropy
                 if isinstance(output, tuple):
                     ofm_re, ofm_aug = output
                     if args.amv_loss == 'error':
                         loss = torch.nn.functional.mse_loss(ofm_re, ofm_aug)
                     elif args.amv_loss == 'distill':
                         # Access inner model to run backbone
                         if hasattr(model, 'module'):
                             inner_model = model.module.model
                             stem = model.module.stem
                         else:
                         ...
                         # Forward RE (Clean -> RE_CONV -> Backbone)
                         stem.forward = lambda x: ofm_re
                         # input is [Aug, Clean, Aug, Clean...]. 
                         # We need Clean images which are at odd indices.
                         clean_imgs = input[1::2]
                         logits_re = inner_model(clean_imgs)
                         
                         # Forward AUG (Aug -> ORI_CONV -> Backbone)
                         stem.forward = lambda x: ofm_aug
                         # Aug images are at even indices.
                         aug_imgs = input[0::2]
                         logits_aug = inner_model(aug_imgs)
                         ...
                         log_probs_re = torch.log_softmax(logits_re, dim=-1)
                         probs_aug = torch.softmax(logits_aug, dim=-1)
                         loss = torch.nn.functional.kl_div(log_probs_re, probs_aug, reduction='batchmean')
                         
                     # Update metrics
                     # AMV doesn't have accuracy
                     acc1 = torch.tensor(0.0, device=device)
                     acc5 = torch.tensor(0.0, device=device)
                     
                     # Check batch size consistency
                     # output is [B/2, ...]
                     # target might be [B] (full batch targets including aug)
                     # But utils.accuracy / logging might use output.shape[0] for batch size
                     # We need to ensure losses_m update uses correct batch size
                     # output.shape[0] is correct for the loss (it's average over B/2 samples)
                     # But target is [B].
            ...
            # Fix batch_size for metric update
            # In AMV mode, output.shape[0] is B/2 (number of pairs)
            # reduced_loss is scalar mean.
            # We want to log the loss per pair? Yes.
            batch_size = output[0].shape[0] if isinstance(output, tuple) else output.shape[0]
