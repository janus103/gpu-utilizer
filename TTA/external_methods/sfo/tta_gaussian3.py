...
group.add_argument('--lbatch', type=int, default=0, metavar='M',
                   help='Number of adaptation steps per mini-batch (0=eval only, N=adapt N times then eval)')
...
def main():
    ...
    try:
        eval_metrics = adapt_and_evaluate(
            model,
            loader_eval,
            optimizer,
            validate_loss_fn,
...
def adapt_and_evaluate(
    ...
    """Per-batch TTA: for each mini-batch, optionally adapt N times then evaluate once.

    When lbatch=0, no adaptation is performed and this behaves identically to
    tta_gaussian2.py with lbatch=0 (pure evaluation).
    When lbatch=N, each mini-batch is adapted N times (g_loss only) then evaluated.
    After each mini-batch, the model is reset to its initial state.
    """
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if n_adapt > 0:
            model.train()
            for _adapt_step in range(n_adapt):
                optimizer.zero_grad()
                with amp_autocast():
                    _output = model(input)
                    base_model = _get_base_model(model)

                    g_loss = torch.tensor(0.0, device=device, requires_grad=True)
                    nll_loss = base_model.get_nll_loss()
                    if nll_loss is not None:
                        g_loss = g_loss + args.gaussian_loss_weight * nll_loss
                    nll_sam_loss = base_model.get_nll_sam_loss()
                    if nll_sam_loss is not None:
                        g_loss = g_loss + args.sam_loss_weight * nll_sam_loss
            ...
            gaussian_losses_m.update(g_loss.item(), input.shape[0])
        ...
        if utils.is_primary(args) and (last_batch or batch_idx % args.log_interval == 0):
            adapt_str = f'G-Loss: {gaussian_losses_m.val:#.3g} ({gaussian_losses_m.avg:#.3g})  ' if n_adapt > 0 else ''
            _logger.info(
