...
def parameters_to_vector(model):
    '''transform the model parameters that require gradients into a vector'''
    params_with_grad = [p for p in model.parameters() if p.requires_grad]

    # convert to a vector
    param_vector = torch.nn.utils.parameters_to_vector(params_with_grad)
    return param_vector

def vector_to_parameters(vector, model):
    '''Update the parameters that require gradients with the given vector values'''
