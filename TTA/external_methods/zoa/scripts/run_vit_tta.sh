#!/bin/bash
###############################################################################
# ViT-B TTA Runner — 15 corruptions → CSV
#
# Usage:
#   bash scripts/run_vit_tta.sh <GPU> <MODE> <BIT> <PRETRAINED_PATH>
#
# Arguments:
#   GPU             GPU device number (e.g. 0, 1, 2, 3)
#   MODE            Algorithm name (e.g. no_adapt, zoa_vit, foa, t3a, bn_adapt)
#   BIT             Quantization bit-width (32 = FP32, otherwise enables --quant)
#   PRETRAINED_PATH Path to pretrained model checkpoint
#
# Examples:
#   bash scripts/run_vit_tta.sh 0 zoa_vit 8 ~/NIPS/jin/SOA/pytorch-image-models/ZOA_WEIGHT/ZOA_vit_base_timm_format.pth
#   bash scripts/run_vit_tta.sh 1 no_adapt 32 ~/NIPS/jin/SOA/pytorch-image-models/ZOA_WEIGHT/ZOA_vit_base_timm_format.pth
###############################################################################
...
if [ $# -lt 4 ]; then
    ...
    echo "  MODE : algorithm name (no_adapt, zoa_vit, foa, t3a, bn_adapt, ...)"
...
TAG="vit_base_${MODE}_${PRECISION}"
...
for CORRUPT in "${CORRUPTIONS[@]}"; do
    ...
    CMD+=" --lr 0.0005 --sc 0.02 --lambda_bp 30 --domain_t 0.1"
    CMD+=" ${QUANT_ARGS}"
