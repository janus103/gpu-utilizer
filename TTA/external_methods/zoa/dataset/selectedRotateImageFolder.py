...
# We'll still detect & report the first problematic file once (see _load_image_with_one_time_report).
ImageFile.LOAD_TRUNCATED_IMAGES = True


def _report_bad_image_once(path: str, err: Exception) -> None:
	"""Report a bad image path only once across all DataLoader worker processes.

	The sentinel path is provided by `main.py` via env var `ZOA_BAD_IMAGE_SENTINEL`.
	"""
	sentinel = os.environ.get("ZOA_BAD_IMAGE_SENTINEL")
	if not sentinel:
		# Fallback: print every time if sentinel isn't configured.
		print(f"[ZOA] Corrupted/truncated image: {path}\n[ZOA] Error: {err}", flush=True)
		return

	# Atomic create to ensure only one worker prints.
	try:
		fd = os.open(sentinel, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o644)
...
def _load_image_with_one_time_report(loader, path: str):
	"""Load image; on first failure, report path once and retry with truncated load enabled."""
	sentinel = os.environ.get("ZOA_BAD_IMAGE_SENTINEL")
	# If we've already reported one bad image this run, avoid extra checking.
	if sentinel and os.path.exists(sentinel):
		return loader(path)

	# Try strict load once to detect truncated images, then fall back to tolerant mode.
	try:
		ImageFile.LOAD_TRUNCATED_IMAGES = False
		return loader(path)
	except Exception as e:
		_report_bad_image_once(path, e)
		ImageFile.LOAD_TRUNCATED_IMAGES = True
		return loader(path)
	finally:
		# Keep tolerant mode on for the rest of the run.
		ImageFile.LOAD_TRUNCATED_IMAGES = True


normalize_in1k = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
...
tr_transforms = transforms.Compose([transforms.RandomResizedCrop(224),
									...
									transforms.ToTensor(),
									normalize])
# te transform use in1k normalization for original images
...
class ImagePathFolder(datasets.ImageFolder):
	...
	def __getitem__(self, index):
		...
		# Best-effort: report the first corrupted image once and try to keep going.
		# (If the file is too broken to load even with truncated mode, we will raise.)
		img = _load_image_with_one_time_report(self.loader, path)
...
class SelectedRotateImageFolder(datasets.ImageFolder):
    ...
    def __getitem__(self, index):
        ...
        # Best-effort: report the first corrupted image once and try to keep going.
        # If a file is too broken to load even with truncated mode, resample a few times.
        last_exc = None
        for _ in range(10):
            path, target = self.samples[index]
            try:
                img_input = _load_image_with_one_time_report(self.loader, path)
                last_exc = None
                break
            except Exception as e:
                last_exc = e
    ...
    def set_dataset_size(self, subset_size, seed=0):
        num_train = len(self.targets)
        indices = list(range(num_train))
        random.seed(seed)
        random.shuffle(indices)
        # print(indices[:subset_size])
...
def prepare_test_data(args, use_transforms=True, semi=False):
    if args.corruption == 'original' or args.corruption == 'rendition' or args.corruption == 'v2' or args.corruption == 'sketch':
        ...
        # if args.arch.find('resnet') >= 0 and te_transforms_local is not None:
        #     te_transforms_local.transforms[-1] = normalize_in1k
        if args.use_in1k_norm:
            te_transforms_local.transforms[-1] = normalize_in1k
    elif args.corruption in common_corruptions:
        te_transforms_local = te_transforms_imageC if use_transforms else None
        if args.use_in1k_norm_c:
            te_transforms_local.transforms[-1] = normalize_in1k
    else:
    ...
    if args.corruption == 'original':
        ...
        if args.subset_size is not None:
            teset.set_dataset_size(args.subset_size)
    elif args.corruption in common_corruptions:
        if not semi:
