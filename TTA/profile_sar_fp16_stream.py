...
SAR_ROOT = EXTERNAL_METHODS_ROOT / "sar"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Full ImageNet-C SAR AMP FP16 stream profiler.")
    ...
    p.add_argument("--sar-margin-e0", type=float, default=math.log(1000) * 0.40)
    p.add_argument("--forward-equiv-factor", type=float, default=4.0)
    ...
    p.add_argument("--output", default=str(GPU_UTILIZER_ROOT / "Results" / "TTA" / "sar_fp16_stream.csv"))
    return p.parse_args()
...
def main() -> None:
    ...
    print(f"wrote SAR FP16 stream results: {output}")
...
def run_case(args, metric_config: MetricConfig, model_name: str, batch_size: int, device) -> Dict[str, object]:
    ...
    try:
        ...
        adapt_model = build_sar_model(args, model_name, device)
        ...
        print(f"profiled SAR {model_name} bs={batch_size} samples={row['num_samples']} status={row['status']}")
    except Exception as exc:
        row["status"] = "error"
        row["failure_reason"] = classify_failure(exc)
        row["error"] = repr(exc)
        torch.cuda.empty_cache()
        print(f"profiled SAR {model_name} bs={batch_size} status=error reason={row['failure_reason']}")
    finally:
        torch.cuda.empty_cache()
    return row


def build_sar_model(args, model_name: str, device):
    import timm

    sar = load_module("vendored_sar", SAR_ROOT / "sar.py")
    sam = load_module("vendored_sam", SAR_ROOT / "sam.py")
    model = timm.create_model(model_name, pretrained=args.pretrained, num_classes=1000).to(device)
    model = sar.configure_model(model)
    params, _names = sar.collect_params(model)
    if not params:
        raise RuntimeError(f"SAR found no adaptable norm parameters for {model_name}")
    base_optimizer = torch.optim.SGD
    optimizer = sam.SAM(params, base_optimizer, lr=args.lr, momentum=args.momentum)
    return sar.SAR(model, optimizer, margin_e0=args.sar_margin_e0)
